...
let transfer =
  ((src, dst, amount_, storage) : (address, address, nat, storage)) => {
      let src_balance_opt =
        is_nat(lookup(src, storage.ledger) - amount_);
      switch(src_balance_opt) {
      | Some(src_balance) => {
          let dst_balance = lookup(dst, storage.ledger) + amount_;
          let ledger1 =
              Map.update(dst, Some(dst_balance), storage.ledger);
          let new_ledger = 
              Map.update(src, Some(src_balance), ledger1);
          {...storage, ledger: new_ledger }
        }

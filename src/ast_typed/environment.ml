...
let make_element : type_expression -> full_environment -> environment_element_definition -> element =
  fun type_expression source_environment definition -> {type_expression ; source_environment ; definition}
...
module Small = struct
  ...
  let add_type : string -> type_expression -> t -> t = fun k v -> map_type_environment (fun x -> (k , v) :: x)
  let get_opt : string -> t -> element option = fun k x -> List.assoc_opt k (get_environment x)
  let get_type_opt : string -> t -> type_expression option = fun k x -> List.assoc_opt k (get_type_environment x)
end
...
let add_ez_binder : string -> type_expression -> t -> t = fun k v e ->
...
let add_type : string -> type_expression -> t -> t = fun k v -> List.Ne.hd_map (Small.add_type k v)
let get_opt : string -> t -> element option = fun k x -> List.Ne.find_map (Small.get_opt k) x
let get_type_opt : string -> t -> type_expression option = fun k x -> List.Ne.find_map (Small.get_type_opt k) x

let get_constructor : string -> t -> (type_expression * type_expression) option = fun k x -> (* Left is the constructor, right is the sum type *)
  let aux = fun x ->
    let aux = fun (_type_name , (x : type_expression)) ->
      match x.type_expression' with
...
module PP = struct
  ...
  let environment_element = fun ppf (k , (ele : environment_element)) ->
    fprintf ppf "%s -> %a" k PP.type_expression ele.type_expression

  let type_environment_element = fun ppf (k , tv) ->
    fprintf ppf "%s -> %a" k PP.type_expression tv

...
let failwith_to_string (f : run_failwith_res) : string result =
  let%bind str =
    ...
    | Failwith_bytes b  -> ok @@ Format.asprintf "0X%a" Hex.pp (Hex.of_bytes b)
  in
...
let make_dry_run_options (opts : dry_run_options) : options result =
  ...
  let%bind predecessor_timestamp =
    ...
    | Some st -> (
      ...
      with
      | Some t -> ok (Some t)
      | None   -> simple_fail ("\"" ^ st ^ "\" is a bad timestamp notation") )
  in
...
let run_contract
  ...
  let%bind descr =
    ...
    @@ Memory_proto_alpha.parse_michelson_fail ~top_level exp ty_stack_before
         ty_stack_after
  in

...
module Errors = struct
  ...
  let failwith str () =
    ...
    let message () = "" in
    let data = [("value", fun () -> Format.asprintf "%s" str)] in
...
type run_res = Success of ex_typed_value | Fail of run_failwith_res
...
let failwith_to_string (f : run_failwith_res) : string result =
  let%bind str =
    ...
    | Failwith_int i ->
        ok @@ string_of_int i
    | Failwith_string s ->
        ok @@ Format.asprintf "\"%s\"" (String.escaped s)
    | Failwith_bytes b ->
        ok @@ Format.asprintf "0X%a" Hex.pp (Hex.of_bytes b)
  in
...
let make_dry_run_options (opts : dry_run_options) : options result =
  ...
  let%bind predecessor_timestamp =
    ...
    | Some st -> (
      match
        Memory_proto_alpha.Protocol.Alpha_context.Timestamp.of_notation st
      with
      | Some t ->
          ok (Some t)
      | None ->
          simple_fail ("\"" ^ st ^ "\" is a bad timestamp notation") )
  in
  ok @@ make_options ?predecessor_timestamp ~amount ~balance ?sender ?source ()
...
let fetch_lambda_types (contract_ty : ex_ty) =
  ...
  | Ex_ty (Lambda_t (in_ty, out_ty, _)) ->
      ok (Ex_ty in_ty, Ex_ty out_ty)
  | _ ->
      simple_fail "failed to fetch lambda types"

let run_contract ?options (exp : Michelson.t) (exp_type : ex_ty)
    (input_michelson : Michelson.t) : run_res result =
  ...
  | Memory_proto_alpha.Fail expr -> (
    match
      ...
      @@ Memory_proto_alpha.strings_of_prims expr
    with
    | Int (_, i) ->
        ok @@ Fail (Failwith_int (Z.to_int i))
    | String (_, s) ->
        ok @@ Fail (Failwith_string s)
    | Bytes (_, s) ->
        ok @@ Fail (Failwith_bytes s)
    | _ ->
        fail @@ Errors.unknown_failwith_type () )

let run_expression ?options (exp : Michelson.t) (exp_type : ex_ty) :
    run_res result =
  let open! Tezos_raw_protocol_006_PsCARTHA in
  ...
  let%bind res =
    Trace.trace_tzresult_lwt (simple_error "error of execution")
    @@ Memory_proto_alpha.failure_interpret ?options descr Empty
  in
  match res with
  | Memory_proto_alpha.Succeed stack ->
      let (Item (output, Empty)) = stack in
...
let run_failwith ?options (exp : Michelson.t) (exp_type : ex_ty) :
    run_failwith_res result =
  let%bind expr = run_expression ?options exp exp_type in
  match expr with
  | Success _ ->
      simple_fail "An error of execution was expected"
  | Fail res ->
      ok res

let run_no_failwith ?options (exp : Michelson.t) (exp_type : ex_ty) :
    ex_typed_value result =
  let%bind expr = run_expression ?options exp exp_type in
  match expr with
  | Success tval ->
      ok tval

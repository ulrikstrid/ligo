...
let type_expression source_file syntax expression env state =
  let%bind v_syntax =
    Helpers.syntax_to_variant (Syntax_name syntax) source_file
  in
...
let compile_and_aggregate_expression source_file syntax expression env state
  ...
  in
  let%bind compiled =
    Of_mini_c.aggregate_and_compile_expression mini_c_prg mini_c_exp
  in
  ok @@ compiled
...
let compile_storage storage input source_file syntax env state mini_c_prg =
  ...
  in
  let%bind imperative =
    Of_source.compile_contract_input storage input v_syntax
  in

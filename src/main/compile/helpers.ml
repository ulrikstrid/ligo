...
let parsify_pascaligo source =
  let%bind raw =
    trace (simple_error "parsing") @@ Parser.Pascaligo.parse_file source
  in
  let%bind imperative =
...
let parsify_expression_pascaligo source =
  let%bind raw =
    ...
    @@ Parser.Pascaligo.parse_expression source
  in
  let%bind imperative =
...
let parsify_cameligo source =
  let%bind raw =
    trace (simple_error "parsing") @@ Parser.Cameligo.parse_file source
  in
  let%bind imperative =
    trace (simple_error "abstracting")
...
let parsify_expression_cameligo source =
  let%bind raw =
    trace (simple_error "parsing expression")
    @@ Parser.Cameligo.parse_expression source
  in
  let%bind imperative =
    trace (simple_error "abstracting expression")
...
let parsify_reasonligo source =
  let%bind raw =
    trace (simple_error "parsing") @@ Parser.Reasonligo.parse_file source
  in
  let%bind imperative =
    trace (simple_error "abstracting")
    @@ Concrete_to_imperative.Cameligo.compile_program raw
...
let parsify_expression_reasonligo source =
  let%bind raw =
    trace (simple_error "parsing expression")
    @@ Parser.Reasonligo.parse_expression source
  in
  let%bind imperative =
    trace (simple_error "abstracting expression")
    @@ Concrete_to_imperative.Cameligo.compile_expression raw
...
let parsify_string_reasonligo source =
  let%bind raw =
    trace (simple_error "parsing") @@ Parser.Reasonligo.parse_string source
  in
  let%bind imperative =
    trace (simple_error "abstracting")
    @@ Concrete_to_imperative.Cameligo.compile_program raw
...
let parsify_string_pascaligo source =
  let%bind raw =
    trace (simple_error "parsing") @@ Parser.Pascaligo.parse_string source
  in
  let%bind imperative =
    trace (simple_error "abstracting")
    @@ Concrete_to_imperative.Pascaligo.compile_program raw
...
let parsify_string_cameligo source =
  let%bind raw =
    trace (simple_error "parsing") @@ Parser.Cameligo.parse_string source
  in
  let%bind imperative =
    trace (simple_error "abstracting")
    @@ Concrete_to_imperative.Cameligo.compile_program raw
...
let pretty_print_pascaligo source =
  ...
  let state =
    Parser_pascaligo.ParserLog.mk_state ~offsets:true ~mode:`Byte ~buffer
  in
...
let pretty_print_cameligo source =
  ...
  let buffer = Buffer.create 59 in
  let state =
    (* TODO: Should flow from the CLI *)
    Parser_cameligo.ParserLog.mk_state ~offsets:true ~mode:`Point ~buffer
  in
...
let pretty_print_reasonligo source =
  ...
  let state =
    (* TODO: Should flow from the CLI *)
    Parser.Reasonligo.ParserLog.mk_state ~offsets:true ~mode:`Point ~buffer
  in

...
let compile_contract : expression -> Compiler.compiled_expression result =
 fun e ->
...
let compile_expression : expression -> Compiler.compiled_expression result =
 fun e ->
  let%bind expr =
...
let aggregate_and_compile program form =
...
let aggregate_and_compile_contract (program : Types.program) name =
...
let aggregate_and_compile_expression program exp =
  aggregate_and_compile program (ExpressionForm exp)

let pretty_print program = Mini_c.PP.program program

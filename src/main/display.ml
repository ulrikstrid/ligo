...
let rec error_pp ?(dev = false) out (e : error) =
  ...
  let message =
    ...
    match opt with Some msg -> ": " ^ msg | None -> ""
  in
  let error_code =
  ...
  let title =
    ...
    Option.unopt ~default:"" opt
  in
  let data =
    let data = e |> member "data" in
    match data with `Null -> "" | _ -> " " ^ J.to_string data ^ "\n"
  in
  let infos =
    let infos = e |> member "infos" in
    match infos with `List lst -> lst | `Null -> [] | x -> [x]
  in
  let children =
...
let string_result_pp_json out (r : string result) =
  let status_json status content : J.t =
    `Assoc [("status", `String status); ("content", content)]
  in
  match r with
  | Ok (x, _) ->

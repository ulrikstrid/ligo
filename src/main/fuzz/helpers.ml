...
let rec map_expression : 'err exp_mapper -> expression -> expression monad = fun f e ->
  ...
  | E_ascription ascr -> (
      let%bind ascr = ascription self ok ascr in
  ...
  | E_constructor c -> (
      let%bind c = constructor self c in
  ...
  | E_let_in li -> (
      let%bind li = let_in self ok li in
      return @@ E_let_in li
    )
  | E_type_in ti -> (
      let%bind ti = type_in self ok ti in
      return @@ E_type_in ti
    )
  | E_mod_alias ma -> (
  ...
  | E_mod_in mi -> (
      let%bind mi = mod_in self ok mi in
      return @@ E_mod_in mi
    )
  | E_lambda l -> (
      let%bind l = lambda self ok l in
      return @@ E_lambda l
    )
  | E_recursive r ->
      let%bind r = recursive self ok r in
  ...
  | E_cond c ->
      let%bind c = conditional self c in
      return @@ E_cond c
  | E_sequence s -> (
      let%bind s = sequence self s in
      return @@ E_sequence s
    )
  | E_assign a -> (
      let%bind a = assign self a in
      return @@ E_assign a
  )
  | E_for f ->
      let%bind f = for_ self f in
      return @@ E_for f
  | E_for_each fe ->
      let%bind fe = for_each self fe in
      return @@ E_for_each fe
  | E_while w ->
      let%bind w = while_loop self w in
      return @@ E_while w
...
and map_module : 'err abs_mapper -> module_ -> (module_ ) monad = fun m p ->
  let aux = fun (x : declaration) ->
    ...
    | (Declaration_constant dc, Expression m') -> (
        let%bind dc = declaration_constant (map_expression m') ok dc in
        ok (Declaration_constant dc)
      )
    (* | (Declaration_type dt, Type_expression m') -> (
     *     let%bind dt = declaration_type (map_type_expression m') dt in

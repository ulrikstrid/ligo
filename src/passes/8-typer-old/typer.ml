...
module Errors = struct
  ...
  let fvs_in_create_contract_lambda
      (e : I.expression)
      (fvar : Ast_typed.expression_variable)
      () =
  ...
  let create_contract_lambda (cst : I.constant') (e : I.expression) () =
    let title () =
      Format.asprintf "%a first argument must be inlined" I.PP.constant cst
    in
    let message () =
      Format.asprintf "contract code can be inlined using a lambda"
    in
    let data =
      [ ("expression", fun () -> Format.asprintf "%a" I.PP.expression e);
        ("location", fun () -> Format.asprintf "%a" Location.pp e.location) ]
...
let rec type_program (p : I.program) : (O.program * Solver.state) result =
  let aux
      ((e, acc) : environment * O.declaration Location.wrap list)
      (d : I.declaration Location.wrap) =
    let%bind ed' =
      (bind_map_location
         (type_declaration e (Solver.placeholder_for_state_of_new_typer ())))
        d
    in
    let loc : 'a. 'a Location.wrap -> _ -> _ =
     fun x v -> Location.wrap ~loc:x.location v
    in
    let e', _placeholder_for_state_of_new_typer, d' = Location.unwrap ed' in
    match d' with None -> ok (e', acc) | Some d' -> ok (e', loc ed' d' :: acc)
  in
  let%bind _, lst =
    trace (fun () -> program_error p ())
    @@ bind_fold_list aux (Environment.full_empty, []) p
  in
  ok @@ (List.rev lst, Solver.placeholder_for_state_of_new_typer ())

and type_declaration env (_placeholder_for_state_of_new_typer : Solver.state) :
  ...
  | Declaration_constant (name, tv_opt, inline, expression) ->
      ...
      let%bind ae' =
        ...
        @@ type_expression' ?tv_opt:tv'_opt env expression
      in
      let env' = Environment.add_ez_ae name ae' env in
      ok
        ( env',
          Solver.placeholder_for_state_of_new_typer (),
          Some (O.Declaration_constant (name, ae', inline, env')) )

and type_match :
    (environment -> I.expression -> O.expression result) ->
    environment ->
    O.type_expression ->
    I.matching_expr ->
    I.expression ->
    Location.t ->
    O.matching_expr result =
 fun f e t i ae loc ->
   match i with
...
and evaluate_type (e : environment) (t : I.type_expression) :
  ...
  | T_sum m ->
      let aux k v prev =
        ...
        let%bind () =
          match Environment.get_constructor k e with
 ...
 fun e ?tv_opt ae ->
   ...
   let return expr tv =
     let%bind () =
       match tv_opt with
       | None     -> ok ()
       | Some tv' -> O.assert_type_expression_eq (tv', tv)
     in
     let location = ae.location in
     ok @@ make_a_e ~location expr tv e
   in
   let main_error =
   ...
   | E_literal Literal_void ->
       return (E_literal Literal_void) (t_unit ())
       (* TODO : IS this really a t_unit ?*)
   ...
   | E_record_accessor {record; path} ->
       ...
       let aux (prev : O.expression) (a : I.label) : O.expression result =
         ...
         let%bind tv =
           generic_try
             (bad_record_access property ae prev.type_expression ae.location)
           @@ fun () -> I.LMap.find property r_tv
         in
         let location = ae.location in
         ok
         @@ make_a_e ~location
              ...
              tv e
       in
       let%bind ae = trace (simple_info "accessing") @@ aux e' path in
       (* check type annotation of the final accessed element *)
       let%bind () =
         match tv_opt with
         | None     -> ok ()
         | Some tv' -> O.assert_type_expression_eq (tv', ae.type_expression)
       in
       ok ae
   (* Sum *)
   ...
   | E_record m ->
       ...
       in
       let%bind m' =
         Stage_common.Helpers.bind_fold_lmap aux (ok I.LMap.empty) m
       in
   ...
   | E_record_update {record; path; update} ->
       ...
       let%bind tv =
         ...
         | T_record record -> (
             ...
             | None    ->
                 failwith
                 @@ Format.asprintf "field %a is not part of record %a"
                      Stage_common.PP.label path O.PP.type_expression wrapped )
         | _               -> failwith
   ...
   | E_constant
       { cons_name= (C_LIST_FOLD | C_MAP_FOLD | C_SET_FOLD) as opname;
         arguments=
           [ ( { expression_content=
                   I.E_lambda
                     { binder= lname;
       ...
       let%bind v_col, v_initr =
         bind_map_pair (type_expression' e) (collect, init_record)
       in
       let tv_col = get_type_expression v_col in
       (* this is the type of the collection  *)
       let tv_out = get_type_expression v_initr in
       (* this is the output type of the lambda*)
       let%bind input_type =
         match tv_col.type_content with
         | O.T_operator (TC_list t | TC_set t) ->
             ok @@ make_t_ez_record [("0", tv_out); ("1", t)]
         | O.T_operator (TC_map (k, v) | TC_big_map (k, v)) ->
             ok
             @@ make_t_ez_record
                  [("0", tv_out); ("1", make_t_ez_record [("0", k); ("1", v)])]
         | _ ->
             let wtype =
               Format.asprintf
                 "Loops over collections expect lists, sets or maps, got type \
                  %a"
                 O.PP.type_expression tv_col
             in
             fail @@ simple_error wtype
       in
       let lname = lname in
       ...
       let lambda' =
         make_a_e
           (E_lambda {binder= lname; result= body})
           (t_function input_type output_type ())
           e'
       in
   ...
   | E_constant
       { cons_name= C_FOLD_WHILE as opname;
         arguments=
           [ ( { expression_content=
                   I.E_lambda
                     { binder= lname;
       ...
       let%bind () =
         ...
         | {expression_content= O.E_lambda l; _} :: _ ->
             ...
             if List.length fvs = 0 then
               ...
               fail @@ fvs_in_create_contract_lambda ae (List.hd fvs)
         | _ -> fail @@ create_contract_lambda C_CREATE_CONTRACT ae
       in
       let tv_lst = List.map get_type_expression lst' in
       let%bind name', tv = type_constant cons_name tv_lst tv_opt in
     ...
     ->
       ...
       let tv =
         ...
         | None    -> (
           ...
           | _         ->
               failwith
                 "Only C_SET_ADD and C_CONS are possible because those were \
                  the two cases matched above" )
       in
   ...
   | E_application {lamb; args} ->
       ...
       let%bind tv =
         match lamb'.type_expression.type_content with
         | T_arrow {type1; type2} ->
             let%bind _ =
               O.assert_type_expression_eq (type1, args'.type_expression)
             in
             ok type2
         | _ ->
             fail
             @@ type_error_approximate ~expected:"should be a function type"
                  ~expression:lamb ~actual:lamb'.type_expression lamb'.location
       in
   ...
   | E_matching {matchee; cases} ->
       ...
       let%bind m' =
         type_match
           (type_expression' ?tv_opt:None)
           e ex'.type_expression cases ae ae.location
       in
       let tvs =
       ...
       let aux prec cur =
         let%bind () =
           match prec with
           | None      -> ok ()
           | Some cur' -> Ast_typed.assert_type_expression_eq (cur, cur')
         in
         ok (Some cur)
       in
       let%bind tv_opt = bind_fold_list aux None tvs in
       let%bind tv =
         trace_option (match_empty_variant cases ae.location) @@ tv_opt
       in
   ...
   | E_ascription {anno_expr; type_annotation} ->
       ...
       let%bind type_annotation =
         O.merge_annotation (Some tv) (Some expr'.type_expression)
           (internal_assertion_failure
              "merge_annotations (Some ...) (Some ...) failed")
       in
       (* check type annotation of the expression as a whole (e.g. let x : t = (v : t') ) *)
       let%bind () =
         match tv_opt with
         | None     -> ok ()
         | Some tv' -> O.assert_type_expression_eq (tv', type_annotation)
       in
       ok {expr' with type_expression= type_annotation}

and type_lambda e {binder; input_type; output_type; result} =
  let%bind input_type =
    let%bind input_type =
      (* Hack to take care of let_in introduced by `simplify/cameligo.ml` in ECase's hack *)
      let default_action e () =
        fail @@ needs_annotation e "the returned value"
      in
...
and untype_expression_content ty (ec : O.expression_content) :
    I.expression result =
  let open I in
  ...
  | E_lambda {binder; result} ->
      ...
      let%bind input_type, output_type =
        bind_map_pair untype_type_expression io
      in
  ...
  | E_recursive {fun_name; fun_type; lambda} ->
      ...
      let lambda =
        match unty_expr.expression_content with
        | I.E_lambda l -> l

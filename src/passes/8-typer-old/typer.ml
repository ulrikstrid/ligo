...
module Errors = struct
  ...
  let fvs_in_create_contract_lambda (e : I.expression)
  ...
  let type_error_approximate ?(msg = "") ~(expected : string)
  ...
  let bad_record_access (field : I.label) (ae : I.expression)
      (t : O.type_expression) (loc : Location.t) () =
...
let rec type_program (p : I.program) : (O.program * Solver.state) result =
  let aux ((e, acc) : environment * O.declaration Location.wrap list)
  ...
  | T_sum m ->
      let aux k v prev =
        ...
        let%bind () =
          ...
          | Some _ ->
              fail (redundant_constructor e k)
          | None ->
              ok ()
        in
 ...
 fun e ?tv_opt ae ->
  ...
  | E_record_accessor {record; path} ->
      ...
      let%bind () =
        ...
        | None ->
            ok ()
        | Some tv' ->
            O.assert_type_expression_eq (tv', ae.type_expression)
      in
      ok ae
  (* Sum *)
      ...
      let%bind tv =
        ...
        | T_record record -> (
            ...
            | None ->
                failwith
                @@ Format.asprintf
                     "field %a is not part of record %a"
  ...
  | E_constant
      ...
      let%bind input_type =
        ...
        | _ ->
            let wtype =
              Format.asprintf
                "Loops over collections expect lists, sets or maps, got type %a"
                O.PP.type_expression
                tv_col
            in
  ...
  | E_constant {cons_name = C_CREATE_CONTRACT as cons_name; arguments} ->
      ...
      let%bind () =
        ...
        | _ ->
            fail @@ create_contract_lambda C_CREATE_CONTRACT ae
      in
      let tv_lst = List.map get_type_expression lst' in
  ...
  | E_constant {cons_name = C_MAP_ADD as cst; arguments = [key; value; map]} ->
      ...
      let tv =
        ...
        | Some tv ->
            tv
        | None ->
            t_map_or_big_map tv_key tv_val ()
      in
      ...
      let%bind (name', tv) = type_constant cst tv_lst tv_opt in
      return
        (E_constant {cons_name = name'; arguments = [key'; val'; map']})
        tv
      ...
      let aux prec cur =
        let%bind () =
          ...
          | None ->
              ok ()
          | Some cur' ->
              Ast_typed.assert_type_expression_eq (cur, cur')
        in
        ok (Some cur)
      in
  ...
  | E_ascription {anno_expr; type_annotation} ->
      ...
      let%bind () =
        ...
        | None ->
            ok ()
        | Some tv' ->
            O.assert_type_expression_eq (tv', type_annotation)
      in

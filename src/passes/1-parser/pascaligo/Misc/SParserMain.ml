...
let file = match options.input with None | Some "-" -> false | Some _ -> true
...
let log =
  Log.output_token ~offsets:options.offsets options.mode options.cmd cout
...
let () =
  try
    ...
    if Utils.String.Set.mem "ast" options.verbose then (

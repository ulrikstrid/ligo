...
module Ord = struct
  ...
  let compare v1 v2 = compare v1.value v2.value
end
...
let reserved =
  let open SSet in
  empty |> add "get_force" |> add "get_chain_id" |> add "transaction"
  |> add "get_contract" |> add "get_entrypoint" |> add "size" |> add "int"
  |> add "abs" |> add "is_nat" |> add "amount" |> add "balance" |> add "now"
  |> add "unit" |> add "source" |> add "sender" |> add "failwith"
  |> add "bitwise_or" |> add "bitwise_and" |> add "bitwise_xor"
  |> add "string_concat" |> add "string_slice" |> add "crypto_check"
  |> add "crypto_hash_key" |> add "bytes_concat" |> add "bytes_slice"
  |> add "bytes_pack" |> add "bytes_unpack" |> add "set_empty" |> add "set_mem"
  |> add "set_add" |> add "set_remove" |> add "set_iter" |> add "set_fold"
  |> add "list_iter" |> add "list_fold" |> add "list_map" |> add "map_iter"
  |> add "map_map" |> add "map_fold" |> add "map_remove" |> add "map_update"
  |> add "map_get" |> add "map_mem" |> add "sha_256" |> add "sha_512"
  |> add "blake2b" |> add "cons" |> add "address" |> add "self_address"
  |> add "implicit_account" |> add "set_delegate"
...
let check_reserved_name var =
  if SSet.mem var.value reserved then raise (Error (Reserved_name var))
...
let rec vars_of_pattern env = function
  | PConstr p ->
      vars_of_pconstr env p
  | PWild _ | PInt _ | PNat _ | PBytes _ | PString _ ->
      env
  | PList l ->
      vars_of_plist env l
  | PTuple t ->
      vars_of_ptuple env t.value
  | PVar var ->
      if VarSet.mem var env then raise (Error (Non_linear_pattern var))
...
and vars_of_pinj env inj = Utils.sepseq_foldl vars_of_pattern env inj.elements
...
let check_pattern p = check_linearity p |> check_reserved_names |> ignore
...
let check_variants variants =
  let add acc {value; _} =
    ...
    else VarSet.add value.constr acc
  in
  let variants = List.fold_left add VarSet.empty variants in
  ignore variants
...
let check_parameters params =
  ...
  in
  let params = List.fold_left add VarSet.empty params in
  ignore params
...
let check_fields fields =
  let add acc {value; _} =
    ...
    else VarSet.add value.field_name acc
  in
  let fields = List.fold_left add VarSet.empty fields in
  ignore fields

...
let print_csv state print {value; _} = print_nsepseq state "," print value
...
and print_let_binding state {binders; lhs_type; eq; let_rhs} =
  ...
  let () =
    ...
    | Some (colon, type_expr) ->
        ...
        print_type_expr state type_expr
  in
  let () = print_token state eq "=" in
  print_expr state let_rhs

and print_pattern state = function
  | PTuple ptuple -> print_csv state print_pattern ptuple
  ...
  | PRecord r -> print_record_pattern state r
  | PTyped t -> print_typed_pattern state t
...
and print_raw state {value= p1, c, p2; _} =
  print_pattern state p1 ; print_token state c "::" ; print_pattern state p2
...
and print_none_pattern state value = print_token state value "None"
...
and print_constr_app_pattern state node =
  ...
  print_uident state constr ;
  match p_opt with None -> () | Some pattern -> print_pattern state pattern
...
and print_constr_app_expr state {value; _} =
  ...
  match argument with None -> () | Some arg -> print_expr state arg

and print_expr_par state {value; _} =
  let {lpar; inside= e; rpar} = value in
  print_token state lpar "(" ; print_expr state e ; print_token state rpar ")"

and print_unit state {value= lpar, rpar; _} =
  print_token state lpar "(" ; print_token state rpar ")"
...
and print_list_expr state = function
  ...
  | EListComp e ->
      if e.value.elements = None then
        print_token state e.region "[]"
      else
        print_injection state print_expr e
...
and print_bool_expr state = function
  ...
  | True kwd_true -> print_token state kwd_true "true"
  | False kwd_false -> print_token state kwd_false "false"
...
and print_record_expr state e = print_ne_injection state print_field_assign e
...
and print_sequence state seq = print_injection state print_expr seq
...
and print_fun_expr state {value; _} =
  ...
  let () =
    ...
    | Some (colon, type_expr) ->
        ...
        print_type_expr state type_expr
  in
  let () = print_token state arrow "->" in
  print_expr state body

and print_conditional state {value; _} =
  let {kwd_if; test; kwd_then; ifso; kwd_else; ifnot} = value in
...
let expr_to_string ~offsets ~mode = to_string ~offsets ~mode print_expr
...
and pp_let_binding state node attr =
  ...
  let arity =
    ...
    pp_node state "<binders>" ; pp_binders state binders ; 0
  in
  let arity =
    match lhs_type with
    | None                -> arity
...
and pp_binders state patterns =
  ...
  List.iteri (apply arity) patterns
...
and pp_list_pattern state = function
  ...
  | PListComp {value; region} ->
      ...
      if value.elements = None then
        pp_node (state#pad 1 0) "<nil>"
      else
        pp_injection pp_pattern state value
...
and pp_constr_app_pattern state (constr, pat_opt) =
  ...
  match pat_opt with None -> () | Some pat -> pp_pattern state pat
...
and pp_expr state = function
  ...
  | EUnit u -> pp_loc_node state "EUnit" u.region
  | ETuple e_tuple ->
...
and pp_let_in state node =
  ...
  let arity =
    let state = state#pad fields 0 in
    pp_node state "<binders>" ; pp_binders state binders ; arity
  in
  let arity =
    match lhs_type with
    | None                -> arity
  ...
  let () =
    if attributes <> [] then (
      ...
      let apply len rank = pp_ident (state#pad len rank) in
      List.iteri (apply length) attributes )
  in
  ()
...
and pp_selection state = function
  ...
  | Component c  -> pp_node state "Component" ; pp_int state c
...
and pp_constr_expr state = function
  | ENone region -> pp_loc_node state "ENone" region
...
and pp_case : 'a. (state -> 'a -> unit) -> state -> 'a case -> unit =
 fun printer state case ->
   ...
   let apply len rank = pp_case_clause printer (state#pad len (rank + 1)) in
...
and pp_type_expr state = function
  ...
  | TSum {value; region} ->
      ...
      let apply len rank variant =
        ...
        pp_variant state variant.value
      in

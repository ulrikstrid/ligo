...
and print_type_expr state = function
  | TProd prod ->
      print_cartesian state prod
  | TSum {value; _} ->
      print_nsepseq state "|" print_variant value
  | TRecord t ->
      print_rec_type_expr state t
  | TApp app ->
      print_type_app state app
  | TPar par ->
      print_type_par state par
  | TVar var ->
      print_var state var
  | TFun t ->
      print_fun_type state t
...
and print_list_pattern state = function
  | PListComp p ->
      print_injection state print_pattern p
  | PCons p ->
      print_raw state p
...
and print_constr_pattern state = function
  | PNone p ->
      print_none_pattern state p
  | PSomeApp p ->
      print_some_app_pattern state p
  | PConstrApp p ->
      print_constr_app_pattern state p
...
and print_expr state = function
  | ELetIn let_in ->
      print_let_in state let_in
  | ECond cond ->
      print_conditional state cond
  | ETuple tuple ->
      print_csv state print_expr tuple
  | ECase case ->
      print_match_expr state case
  | EFun e ->
      print_fun_expr state e
  | EAnnot e ->
      print_annot_expr state e
  | ELogic e ->
  ...
  | EVar v ->
      print_var state v
  | EProj p ->
      print_projection state p
  | EUpdate u ->
      print_update state u
  | EUnit e ->
      print_unit state e
  | EBytes b ->
      print_bytes state b
  | EPar e ->
      print_expr_par state e
  | EList e ->
      print_list_expr state e
  | ESeq seq ->
      print_sequence state seq
  | ERecord e ->
...
and print_constr_expr state = function
  ...
  | ESomeApp e ->
      print_some_app_expr state e
  | EConstrApp e ->
      print_constr_app_expr state e
...
and print_list_expr state = function
  ...
  | EListComp e ->
      if e.value.elements = None then print_token state e.region "[]"
...
and pp_pattern state = function
  ...
  | PInt i ->
      pp_node state "PInt" ; pp_int state i
  | PNat n ->
...
and pp_list_pattern state = function
  ...
  | PListComp {value; region} ->
      ...
      if value.elements = None then pp_node (state#pad 1 0) "<nil>"
...
and pp_selection state = function
  ...
  | Component c ->
      pp_node state "Component" ; pp_int state c

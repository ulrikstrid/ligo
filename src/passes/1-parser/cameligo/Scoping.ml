...
let reserved =
  let open SSet in
  empty |> add "abs" |> add "address" |> add "amount" |> add "assert"
  |> add "balance" |> add "black2b" |> add "check" |> add "continue"
  |> add "failwith" |> add "gas" |> add "hash" |> add "hash_key"
  |> add "implicit_account" |> add "int" |> add "pack" |> add "self_address"
  |> add "sender" |> add "sha256" |> add "sha512" |> add "source" |> add "stop"
  |> add "time" |> add "unit" |> add "unpack"
...
let rec vars_of_pattern env = function
  ...
  | PList l ->
      vars_of_plist env l
  | PTuple t ->
      Utils.nsepseq_foldl vars_of_pattern env t.value
  | PPar p ->
      vars_of_pattern env p.value.inside
  | PRecord p ->
      vars_of_fields env p.value.ne_elements
  | PTyped p ->
      vars_of_pattern env p.value.pattern
...
and vars_of_field_pattern env field =
  ...
  if VarSet.mem var env then raise (Error (Non_linear_pattern var))
    ...
    vars_of_pattern (VarSet.add var env) p

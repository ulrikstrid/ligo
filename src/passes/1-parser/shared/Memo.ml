...
let check_phase = function
  | Parser (Some valid_lexeme, invalid_lexeme) ->
      ...
      then
        Stdlib.Error Ghost_lexeme
      else if valid_lexeme.value = "" then
        Stdlib.Error Empty_lexeme
      else
        Ok ()
  | Parser (None, invalid_lexeme) ->
      if (invalid_lexeme.region)#is_ghost then
        Stdlib.Error Ghost_lexeme
      else
        Ok ()
  | Lexer | Scoping -> Ok ()
...
let string_of_entry ~(file : bool) entry : string =
  ...
  let string =
    ...
    | Parser (None, invalid_lexeme)                -> (
      ...
      | lexeme -> sprintf "Parse error %s, before \"%s\"" reg lexeme )
    | Parser (Some valid_lexeme, invalid_lexeme) -> (
        let string =

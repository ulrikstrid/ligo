...
type location = Loc of Region.t (* Not ghost *) | Link of macro
...
type phase = Lexer | Parser of valid_lexeme option * invalid_lexeme | Scoping
...
let check_loc = function
  | Loc reg ->
      if reg#is_ghost then Stdlib.Error Ghost_region else Ok ()
  | Link {origin; _} ->
      if origin#is_ghost then Stdlib.Error Ghost_region else Ok ()
...
type entry = < phase : phase ; kind : kind >

type invalid_entry = Ghost_lexeme | Empty_lexeme

let check_phase = function
  | Parser (Some valid_lexeme, invalid_lexeme) ->
      let open Region in
      if (valid_lexeme.region)#is_ghost || (invalid_lexeme.region)#is_ghost
      then Stdlib.Error Ghost_lexeme
      else if valid_lexeme.value = "" then Stdlib.Error Empty_lexeme
      else Ok ()
  | Parser (None, invalid_lexeme) ->
      if (invalid_lexeme.region)#is_ghost then Stdlib.Error Ghost_lexeme
      else Ok ()
  | Lexer | Scoping ->
      Ok ()
...
let string_of_entry ~(file : bool) entry : string =
  let reg = (entry#region)#to_string ~file ~offsets:entry#offsets error#mode in
  let string =
    ...
    | Parser (None, invalid_lexeme) -> (
      ...
      | lexeme ->
          sprintf "Parse error %s, before \"%s\"" reg lexeme )
    | Parser (Some valid_lexeme, invalid_lexeme) -> (
        let string =
          sprintf "Parse error %s, after \"%s\"" reg valid_lexeme.Region.value
        in
        match invalid_lexeme.Region.value with
        | "" ->
            string (* EOF *)
        | lexeme ->
            sprintf "%s and before \"%s\"" string lexeme )
    | Lexer ->
        sprintf "Lexical error %s" reg
    | Scoping ->
        sprintf "Scoping error %s" reg
  in
  let string =
    string ^ (if error#message = "" then "." else ":\n" ^ error#message) ^ "\n"
  in
  let string =
    string ^ if error#hint = "" then "" else sprintf "Hint: %s\n" error#hint
  in
  string

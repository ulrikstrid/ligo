...
module type PARSER = sig
  ...
  module Incremental : sig
    val interactive_expr : Lexing.position -> expr MenhirInterpreter.checkpoint

    val contract : Lexing.position -> ast MenhirInterpreter.checkpoint
  end
end
...
module Make
    (IO : IO)
    (Lexer : Lexer.S)
    (Parser : PARSER with type token = Lexer.Token.token) (ParErr : sig
...
struct
  ...
  let stack = function I.HandlingError env -> I.stack env | _ -> assert false
  ...
  let incr_contract Lexer.{read; buffer; get_win; close; _} =
    ...
    let ast = I.loop_handle success failure supplier parser in
    close () ; ast
  ...
  let incr_expr Lexer.{read; buffer; get_win; close; _} =
    ...
    let expr = I.loop_handle success failure supplier parser in
    close () ; expr
end

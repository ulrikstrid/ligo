...
and transpile_annotated_expression (ae : AST.expression) : expression result =
      ...
      let iterator_generator iterator_name =
        ...
        fun (lst : AST.expression list) ->
          ...
          | _ ->
              fail
              @@ corner_case ~loc:__LOC__
                   (Format.asprintf "bad iterator arity: %a"
                      Stage_common.PP.constant iterator_name)
      in
      let iter, map, fold =
  ...
  | E_recursive r -> transpile_recursive r
  | E_matching {matchee= expr; cases= m} -> (
      let%bind expr' = transpile_annotated_expression expr in
      match m with
      | Match_bool {match_true; match_false} ->
          let%bind t, f =
            bind_map_pair transpile_annotated_expression
...
and transpile_recursive {fun_name; fun_type; lambda} =
  ...
  and matching :
   ...
   fun fun_name loop_type shadowed m ty ->
     ...
     | Match_option {match_none; match_some= name, s, tv} ->
         let%bind n =
           replace_callback fun_name loop_type shadowed match_none
         in
         let%bind tv', s' =
           let%bind tv' = transpile_type tv in
  ...
  let%bind binder =
    match binder with
    | [hd] -> ok @@ hd
    | _           -> fail @@ unsupported_recursive_function fun_name
  in

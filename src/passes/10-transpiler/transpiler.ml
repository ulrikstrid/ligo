...
module Errors = struct
  let corner_case ~loc message =
    ...
    let content () =
      "we don't have a good error message for this case. we are\n\
       striving find ways to better report them and find the use-cases that \
       generate\n\
       them. please report this to the developers."
    in
    let data = [("location", fun () -> loc); ("message", fun () -> message)] in
  ...
  let bad_big_map location =
    ...
    let content () =
      "only one big_map per program which must appear\n\
      \      on the left hand side of a pair in the contract's storage"
    in
    let data =
      [("location", fun () -> Format.asprintf "%a" Location.pp location)]
    in
  ...
  let unsupported_recursive_function expression_variable =
    ...
    let data =
      [ ( "value",
          fun () ->
            Format.asprintf "%a" AST.PP.expression_variable expression_variable
        ) ]
    in
    error ~data title content
end
...
let rec transpile_type (t : AST.type_expression) : type_value result =
  ...
  | T_constant TC_bytes ->
      ok (T_base TC_bytes)
  | T_constant TC_address ->
      ok (T_base TC_address)
  | T_constant TC_timestamp ->
      ok (T_base TC_timestamp)
  | T_constant TC_unit ->
      ok (T_base TC_unit)
  | T_constant TC_operation ->
      ok (T_base TC_operation)
  | T_constant TC_signature ->
      ok (T_base TC_signature)
  | T_constant TC_key ->
      ok (T_base TC_key)
  | T_constant TC_key_hash ->
      ok (T_base TC_key_hash)
  | T_constant TC_chain_id ->
      ok (T_base TC_chain_id)
  | T_constant TC_void ->
      ok (T_base TC_void)
  ...
  | T_operator (TC_map_or_big_map (_, _)) ->
      ...
      @@ corner_case
           ~loc:"transpiler"
           "TC_map_or_big_map should have been resolved before transpilation"
  ...
  | T_record m ->
      let node =
        Append_tree.of_list
        @@ Stage_common.Helpers.kv_list_of_record_or_tuple m
      in
      let aux a b : type_value annotated result =
        let%bind a = a in
        let%bind b = b in
...
let record_access_to_lr :
    type_value ->
    type_value AST.label_map ->
    AST.label ->
    (type_value * [`Left | `Right]) list result =
 fun ty tym ind ->
...
let rec transpile_literal : AST.literal -> value =
 fun l ->
  match l with
  | Literal_bool b ->
      D_bool b
...
and transpile_environment_element_type :
    AST.environment_element -> type_value result =
 fun ele -> transpile_type ele.type_value

and tree_of_sum :
    AST.type_expression ->
    (AST.constructor' * AST.type_expression) Append_tree.t result =
 fun t ->
...
and transpile_annotated_expression (ae : AST.expression) : expression result =
  ...
  let return ?(tv = tv) expr =
    ok @@ Combinators.Expression.make_tpl (expr, tv)
  in
  let info =
  ...
  | E_literal l ->
      return @@ E_literal (transpile_literal l)
  | E_variable name ->
      let%bind ele =
        trace_option (corner_case ~loc:__LOC__ "name not in environment")
        @@ AST.Environment.get_opt name ae.environment
      in
  ...
  | E_constructor {constructor; element} ->
      ...
      let (param'_expr, param'_tv) =
        Combinators.Expression.(get_content param', get_type param')
      in
      let%bind node_tv =
        trace_strong (corner_case ~loc:__LOC__ "getting lr tree")
        @@ tree_of_sum ae.type_expression
      in
      let leaf (k, tv) : (expression' option * type_value) result =
        if k = constructor then
          let%bind _ =
            trace_strong
              (corner_case ~loc:__LOC__ "wrong type for constructor parameter")
            @@ AST.assert_type_expression_eq (tv, element.type_expression)
          in
      ...
      let node a b : (expression' option * type_value) result =
        ...
        match (a, b) with
        | ((None, a), (None, b)) ->
            ok (None, T_or ((None, a), (None, b)))
        | ((Some _, _), (Some _, _)) ->
            fail
            @@ corner_case
                 ~loc:__LOC__
                 "multiple identical constructors in the same variant"
        | ((Some v, a), (None, b)) ->
            ok
              ( Some
                  (E_constant
                     {
                       cons_name = C_LEFT;
                       arguments = [Combinators.Expression.make_tpl (v, a)];
                     }),
                T_or ((None, a), (None, b)) )
        | ((None, a), (Some v, b)) ->
            ok
              ( Some
                  (E_constant
                     {
                       cons_name = C_RIGHT;
                       arguments = [Combinators.Expression.make_tpl (v, b)];
                     }),
                T_or ((None, a), (None, b)) )
      in
      let%bind (ae_opt, tv) = Append_tree.fold_ne leaf node node_tv in
      let%bind ae =
        trace_option (corner_case ~loc:__LOC__ "inexistant constructor") ae_opt
      in
      return ~tv ae
  | E_record m ->
      (*list_of_lmap to record_to_list*)
      let node =
        Append_tree.of_list @@ Stage_common.Helpers.list_of_record_or_tuple m
      in
  ...
  | E_record_accessor {record; path} ->
      ...
      let%bind ty_lmap =
        ...
        @@ get_t_record (get_type_expression record)
      in
      let%bind ty'_lmap =
        Stage_common.Helpers.bind_map_lmap transpile_type ty_lmap
      in
      let%bind path =
        trace_strong (corner_case ~loc:__LOC__ "record access")
        @@ record_access_to_lr ty' ty'_lmap path
      in
      let aux pred (ty, lr) =
        let c = match lr with `Left -> C_CAR | `Right -> C_CDR in
        Combinators.Expression.make_tpl
          (E_constant {cons_name = c; arguments = [pred]}, ty)
      in
  ...
  | E_constant {cons_name = name; arguments = lst} -> (
      let iterator_generator iterator_name =
        ...
        let expression_to_iterator_body (f : AST.expression) =
          ...
          | E_lambda l ->
              lambda_to_iterator_body f l
          | E_variable v -> (
              let%bind elt =
                trace_option (corner_case ~loc:__LOC__ "missing var")
                @@ AST.Environment.get_opt v f.environment
              in
              ...
              | ED_declaration {expr = f; free_variables = _} -> (
                ...
                | E_lambda l ->
                    lambda_to_iterator_body f l
                | _ ->
                    fail @@ unsupported_iterator f.location )
              | _ ->
                  fail @@ unsupported_iterator f.location )
          | _ ->
              fail @@ unsupported_iterator f.location
        ...
        fun (lst : AST.expression list) ->
          ...
          | ([f; collection; initial], C_FOLD) ->
              ...
              let%bind collection' =
                transpile_annotated_expression collection
              in
          ...
          | _ ->
              ...
              @@ corner_case
                   ~loc:__LOC__
                   (Format.asprintf
                      "bad iterator arity: %a"
                      Stage_common.PP.constant
                      iterator_name)
      in
      let (iter, map, fold) =
        ( iterator_generator C_ITER,
  ...
  | E_matching {matchee = expr; cases = m} -> (
      ...
      | Match_bool {match_true; match_false} ->
          let%bind (t, f) =
            bind_map_pair
              transpile_annotated_expression
      ...
      | Match_list {match_nil; match_cons = (hd_name, tl_name, match_cons, ty)}
        ->
      ...
      | Match_variant (lst, variant) ->
          ...
          let%bind tree' =
            match tree with
            | Empty ->
                fail (corner_case ~loc:__LOC__ "match empty variant")
            | Full x ->
                ok x
          in
          let%bind tree'' =
          ...
          let rec aux top t =
            ...
            | (`Leaf constructor_name, tv) ->
                let%bind ((_, name), body) =
                  trace_option
                    (corner_case ~loc:__LOC__ "missing match clause")
                  @@ List.find_opt
                       (fun ((constructor_name', _), _) ->
            ...
            | (`Node (a, b), tv) ->
                let%bind a' =
                  ...
                  let%bind e =
                    aux (Expression.make (E_variable left_var) a_ty) a
                  in
                  ok ((left_var, a_ty), e)
                in
                let%bind b' =
                  ...
                  let%bind e =
                    aux (Expression.make (E_variable right_var) b_ty) b
                  in
                  ok ((right_var, b_ty), e)
                in
          ...
          @@ aux expr' tree''
      | AST.Match_tuple _ ->
          fail @@ unsupported_pattern_matching "tuple" ae.location )
...
and transpile_recursive {fun_name; fun_type; lambda} =
  ...
  and replace_callback :
   ...
   fun fun_name loop_type shadowed e ->
    ...
    | E_let_in li ->
        ...
        let%bind let_result =
          replace_callback fun_name loop_type shadowed li.let_result
        in
  ...
  and matching :
   ...
   fun fun_name loop_type shadowed m ty ->
    ...
    | Match_bool {match_true; match_false} ->
        let%bind (t, f) =
          bind_map_pair
            (replace_callback fun_name loop_type shadowed)
            (match_true, match_false)
        in
        return @@ E_if_bool (expr, t, f)
    | Match_option {match_none; match_some = (name, s, tv)} ->
    ...
    | Match_list {match_nil; match_cons = (hd_name, tl_name, match_cons, ty)}
      ->
        let%bind nil =
          replace_callback fun_name loop_type shadowed match_nil
        in
        let%bind cons =
          let%bind ty' = transpile_type ty in
          let%bind match_cons' =
            replace_callback fun_name loop_type shadowed match_cons
          in
          ok (((hd_name, ty'), (tl_name, ty')), match_cons')
        in
        return @@ E_if_cons (expr, nil, cons)
    | Match_variant (lst, variant) ->
        ...
        let rec aux top t =
          ...
          | (`Leaf constructor_name, tv) ->
              ...
              in
              let%bind body' =
                replace_callback fun_name loop_type shadowed body
              in
              return @@ E_let_in ((name, tv), false, top, body')
          | (`Node (a, b), tv) ->
              let%bind a' =
        ...
        @@ aux expr tree''
    | AST.Match_tuple _ ->
        failwith "match_tuple not supported"
  in
  ...
  let%bind binder =
    ...
    | [hd] ->
        ok @@ hd
    | _ ->
        fail @@ unsupported_recursive_function fun_name
  in
  let expr = Expression.make_tpl (E_variable binder, input_type) in
  let body =
    Expression.make
      (E_iterator (C_LOOP_LEFT, ((lambda.binder, loop_type), body), expr))
      output_type
  in
...
let transpile_program (lst : AST.program) : program result =
  ...
  in
  let%bind (statements, _) =
    List.fold_left aux (ok ([], Environment.empty)) (temp_unwrap_loc_list lst)
  in
  ok statements
...
let check_storage f ty loc : (anon_function * _) result =
  let rec aux (t : type_value) on_big_map =
    ...
    | T_big_map _ ->
        on_big_map
    | T_pair (a, b) ->
        aux (snd a) true && aux (snd b) false
    | T_or (a, b) ->
        aux (snd a) false && aux (snd b) false
    | T_function (a, b) ->
        aux a false && aux b false
    | T_map (a, b) ->
        aux a false && aux b false
    | T_list a ->
        aux a false
...
let extract_constructor (v : value) (tree : _ Append_tree.t') :
    (string * value * AST.type_expression) result =
...
let extract_tuple (v : value) (tree : AST.type_expression Append_tree.t') :
    (value * AST.type_expression) list result =
  let open Append_tree in

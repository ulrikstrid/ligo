...
module Errors : sig
  ...
  *)
  ...
  val missing_entry_point : string -> error
  val wrong_mini_c_value : string -> value -> error
  val bad_untranspile : string -> value -> error
  val unknown_untranspile : string -> value -> error
end
...
*)
val transpile_annotated_expression : AST.expression -> (expression , [> error]) result
(*
...
val transpile_program : AST.program -> (program , [> error]) result
val check_storage : anon_function -> 'a -> Location.t -> ((anon_function * 'a) , [> error]) result
(*
...
*)
val extract_constructor : value -> ( string * AST.type_expression ) Append_tree.t' -> ((string * value * AST.type_expression) , [> error]) result
val extract_tuple : value -> AST.type_expression Append_tree.t' -> ((value * AST.type_expression) list , [> error]) result
val extract_record : value -> ( string * AST.type_expression ) Append_tree.t' -> (( string * ( value * AST.type_expression)) list , [> error]) result
val untranspile : value -> AST.type_expression -> (AST.expression , [> error]) result

  ...
  let bad_collection expr =
    ...
    let message () =
      Format.asprintf "\nCannot loop over this collection : %a\n"
        I.PP.expression expr
    in
    let data =
      [("location", fun () -> Format.asprintf "%a" Location.pp expr.location)]
    in
...
let repair_mutable_variable_in_matching
    (match_body : O.expression)
    (element_names : O.expression_variable list)
    (env : I.expression_variable) =
  let%bind (dv, fv), mb =
    Self_ast_sugar.fold_map_expression
      (* TODO : these should use Variables sets *)
        (fun ((decl_var, free_var) :
               O.expression_variable list * O.expression_variable list)
             (ass_exp : O.expression) ->
        match ass_exp.expression_content with
        | E_let_in {let_binder; mut= false; rhs; let_result} ->
            let name, _ = let_binder in
            ok
              ( true,
                (name :: decl_var, free_var),
                O.e_let_in let_binder false false rhs let_result )
        | E_let_in {let_binder; mut= true; rhs; let_result} ->
            ...
            if List.mem name decl_var then
              ...
              let free_var =
                if List.mem name free_var then free_var else name :: free_var
              in
              let expr =
                O.e_let_in (env, None) false false
                  (O.e_record_update (O.e_variable env) (Var.to_name name)
                     (O.e_variable name))
                  let_result
              in
              ok
                ( true,
                  (decl_var, free_var),
                  O.e_let_in let_binder false false rhs expr ) )
      ...
      (element_names, []) match_body
  in
  ok @@ ((dv, fv), mb)

and repair_mutable_variable_in_loops
    (for_body : O.expression)
    (element_names : O.expression_variable list)
    (env : O.expression_variable) =
  let%bind (dv, fv), fb =
    Self_ast_sugar.fold_map_expression
      (* TODO : these should use Variables sets *)
        (fun ((decl_var, free_var) :
              ...
              let expr =
                O.e_let_in (env, None) false false
                  (O.e_record_update (O.e_variable env) "0"
                     (O.e_record_update
                        (O.e_record_accessor (O.e_variable env) "0")
                        (Var.to_name name) (O.e_variable name)))
                  let_result
              in
              ok
                ( true,
...
and restore_mutable_variable
    (expr : O.expression -> O.expression_content)
    (free_vars : O.expression_variable list)
    (env : O.expression_variable) =
  let aux (f : O.expression -> O.expression) (ev : O.expression_variable) expr
      =
    f
      (O.e_let_in (ev, None) true false
         (O.e_record_accessor (O.e_variable env) (Var.to_name ev))
         expr)
  in
...
let rec compile_expression : I.expression -> O.expression result =
 fun e ->
   ...
   | I.E_let_in {let_binder; inline; rhs; let_result} ->
       ...
       return
       @@ O.E_let_in
            {let_binder= (binder, ty_opt); mut= false; inline; rhs; let_result}
   ...
   | I.E_map map ->
       let%bind map = bind_map_list (bind_map_pair compile_expression) map in
       return @@ O.E_map map
   | I.E_big_map big_map ->
       let%bind big_map =
         bind_map_list (bind_map_pair compile_expression) big_map
       in
   ...
   | I.E_cond {condition; then_clause; else_clause} ->
       ...
       let%bind (_, free_vars_true), then_clause =
         repair_mutable_variable_in_matching then_clause' [] env
       in
       let%bind (_, free_vars_false), else_clause =
         repair_mutable_variable_in_matching else_clause' [] env
       in
       let then_clause = add_to_end then_clause (O.e_variable env) in
       let else_clause = add_to_end else_clause (O.e_variable env) in
       let free_vars =
         List.sort_uniq Var.compare @@ free_vars_true @ free_vars_false
       in
       if List.length free_vars != 0 then (
         ...
         let return_expr expr =
           O.E_let_in
             { let_binder= (env, None);
               mut= false;
               inline= false;
               rhs= store_mutable_variable free_vars;
               let_result= O.e_let_in (env, None) false false cond_expr @@ expr
             }
         in
         ...
         return
         @@ O.E_cond
              {condition; then_clause= then_clause'; else_clause= else_clause'}
...
and compile_assign {variable; access_path; expression} expr =
  ...
  let aux ((s, e) : O.expression * _) lst =
    ...
    let e' expr =
      let%bind u = update ~loc:s.location s lst expr in
      e u
    in
    ok @@ (s', e')
  in
  let%bind _, rhs =
    bind_fold_list aux (O.e_variable variable, fun e -> ok @@ e) access_path
  in
...
and compile_lambda : I.lambda -> O.lambda result =
 fun {binder; input_type; output_type; result} ->
   ...
   let%bind output_type =
     bind_map_option compile_type_expression output_type
   in
...
and compile_matching : I.matching -> O.expression_content result =
 fun {matchee; cases} ->
   ...
   | I.Match_bool {match_true; match_false} ->
       ...
       let env = Var.fresh () in
       let%bind (_, free_vars_true), match_true =
         repair_mutable_variable_in_matching match_true' [] env
       in
       let%bind (_, free_vars_false), match_false =
         repair_mutable_variable_in_matching match_false' [] env
       in
       ...
       if List.length free_vars != 0 then (
         let match_expr =
           O.e_matching matchee (O.Match_bool {match_true; match_false})
         in
         let return_expr expr =
           O.E_let_in
             { let_binder= (env, None);
               ...
               rhs= store_mutable_variable free_vars;
               let_result=
                 O.e_let_in (env, None) false false match_expr @@ expr }
         in
         ...
         @@ O.E_matching
              { matchee;
                cases=
                  O.Match_bool
                    {match_true= match_true'; match_false= match_false'} }
   | I.Match_option {match_none; match_some} ->
       ...
       let env = Var.fresh () in
       let%bind (_, free_vars_none), match_none =
         repair_mutable_variable_in_matching match_none' [] env
       in
       let%bind (_, free_vars_some), expr =
         repair_mutable_variable_in_matching expr' [n] env
       in
       let match_none = add_to_end match_none (O.e_variable env) in
       let expr = add_to_end expr (O.e_variable env) in
       let free_vars =
         List.sort_uniq Var.compare @@ free_vars_none @ free_vars_some
       in
       if List.length free_vars != 0 then (
         let match_expr =
           O.e_matching matchee
             (O.Match_option {match_none; match_some= (n, expr, tv)})
         in
         let return_expr expr =
           O.E_let_in
             { let_binder= (env, None);
         ...
         @@ O.E_matching
              { matchee;
                cases=
                  O.Match_option
                    {match_none= match_none'; match_some= (n, expr', tv)} }
   | I.Match_list {match_nil; match_cons} ->
       let%bind match_nil' = compile_expression match_nil in
       let hd, tl, expr, tv = match_cons in
       let%bind expr' = compile_expression expr in
       let env = Var.fresh () in
       let%bind (_, free_vars_nil), match_nil =
         repair_mutable_variable_in_matching match_nil' [] env
       in
       let%bind (_, free_vars_cons), expr =
         repair_mutable_variable_in_matching expr' [hd; tl] env
       in
       let match_nil = add_to_end match_nil (O.e_variable env) in
       let expr = add_to_end expr (O.e_variable env) in
       let free_vars =
         List.sort_uniq Var.compare @@ free_vars_nil @ free_vars_cons
       in
       if List.length free_vars != 0 then (
         let match_expr =
           O.e_matching matchee
             (O.Match_list {match_nil; match_cons= (hd, tl, expr, tv)})
         in
         let return_expr expr =
           O.E_let_in
             { let_binder= (env, None);
         ...
         @@ O.E_matching
              { matchee;
                cases=
                  O.Match_list
                    {match_nil= match_nil'; match_cons= (hd, tl, expr', tv)} }
   ...
   | I.Match_variant (lst, tv) ->
       ...
       if List.length free_vars == 0 then (
         ...
         in
         let match_expr =
           O.e_matching matchee @@ O.Match_variant (cases, tv)
         in
         let return_expr expr =
           O.E_let_in
             { let_binder= (env, None);
...
and compile_while I.{condition; body} =
  ...
  let ctrl = O.e_variable binder in
  let%bind for_body = compile_expression body in
  let%bind (_, captured_name_list), for_body =
    repair_mutable_variable_in_loops for_body [] binder
  in
  let for_body = add_to_end for_body ctrl in
  let aux name expr =
    O.e_let_in (name, None) false false
      (O.e_record_accessor
         (O.e_record_accessor (O.e_variable binder) "0")
         (Var.to_name name))
      expr
  in
  let init_rec = O.e_tuple [store_mutable_variable @@ captured_name_list] in
  let restore expr = List.fold_right aux captured_name_list expr in
  ...
  let aux_func =
    ...
    @@ restore
    @@ O.e_cond cond continue_expr stop_expr
  in
    ...
    O.E_let_in
      { let_binder;
        ...
        let_result=
          ...
          @@ O.e_let_in let_binder false false
               (O.e_record_accessor (O.e_variable env_rec) "0")
          @@ expr }
  in
...
and compile_for I.{binder; start; final; increment; body} =
  ...
  let cond =
    I.e_annotation (I.e_constant C_LE [I.e_variable binder; final]) I.t_bool
  in
  ...
  let ctrl =
    O.e_let_in (binder, Some O.t_int) false false
      (O.e_constant C_ADD [O.e_variable binder; step])
    @@ O.e_let_in (env_rec, None) false false
         (O.e_record_update (O.e_variable env_rec) "1" @@ O.e_variable binder)
    @@ continue_expr
  in
  (* Modify the body loop*)
  let%bind body = compile_expression body in
  let%bind (_, captured_name_list), for_body =
    repair_mutable_variable_in_loops body [binder] env_rec
  in
  let for_body = add_to_end for_body ctrl in
  let aux name expr =
    O.e_let_in (name, None) false false
  ...
  let aux_func =
    O.e_lambda env_rec None None
    @@ O.e_let_in (binder, Some O.t_int) false false
         (O.e_record_accessor (O.e_variable env_rec) "1")
    @@ O.e_cond cond (restore for_body) stop_expr
  in
  (* Make the fold_while en precharge the vakye *)
  let loop = O.e_constant C_FOLD_WHILE [aux_func; O.e_variable env_rec] in
  let init_rec =
    O.e_pair (store_mutable_variable captured_name_list) @@ O.e_variable binder
  in
  let%bind start = compile_expression start in
  let let_binder = (env_rec, None) in
  let return_expr expr =
    O.E_let_in
      { let_binder= (binder, Some O.t_int);
        mut= false;
        inline= false;
        rhs= start;
        let_result=
...
and compile_for_each I.{binder; collection; collection_type; body} =
  ...
  let%bind element_names =
    ok
    @@ match snd binder with Some v -> [fst binder; v] | None -> [fst binder]
  in
  let env = Var.fresh () in
  let%bind body = compile_expression body in
  let%bind (_, free_vars), body =
    repair_mutable_variable_in_loops body element_names args
  in
  let for_body =
    add_to_end body @@ O.e_record_accessor (O.e_variable args) "0"
  in
  ...
  let aux name expr =
    O.e_let_in (name, None) false false
      (O.e_record_accessor
         (O.e_record_accessor (O.e_variable args) "0")
         (Var.to_name name))
      expr
  in
  let restore expr = List.fold_right aux free_vars expr in
  let restore =
    match collection_type with
    | Map -> (
      match snd binder with
      | Some v ->
          fun expr ->
            restore
              (O.e_let_in
                 ...
                 false false
                 (O.e_record_accessor
                    (O.e_record_accessor (O.e_variable args) "1")
                    "0")
                 (O.e_let_in (v, None) false false
                    (O.e_record_accessor
                       (O.e_record_accessor (O.e_variable args) "1")
                       "1")
                    expr))
  ...
  let fold expr =
    O.E_let_in
      { let_binder= (env, None);
        mut= false;
        inline= false;
        rhs= O.e_constant op_name [lambda; collect; init_record];
        let_result= expr }
  in
...
let compile_program : I.program -> O.program result =
 fun p -> bind_map_list compile_declaration p

(* uncompiling *)
let rec uncompile_type_expression :
    O.type_expression -> I.type_expression result =
 fun te ->
...
let rec uncompile_expression : O.expression -> I.expression result =
 fun e ->
   ...
   | O.E_let_in {let_binder; inline; rhs; let_result} ->
       ...
       return
       @@ I.E_let_in {let_binder= (binder, ty_opt); inline; rhs; let_result}
   ...
   | O.E_map map ->
       let%bind map = bind_map_list (bind_map_pair uncompile_expression) map in
       return @@ I.E_map map
   | O.E_big_map big_map ->
       let%bind big_map =
         bind_map_list (bind_map_pair uncompile_expression) big_map
       in
...
and uncompile_lambda : O.lambda -> I.lambda result =
 fun {binder; input_type; output_type; result} ->
   let%bind input_type =
     bind_map_option uncompile_type_expression input_type
   in
   let%bind output_type =
     bind_map_option uncompile_type_expression output_type
   in

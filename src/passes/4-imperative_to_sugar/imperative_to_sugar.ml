...
let rec add_to_end (expression : O.expression) to_add =
  ...
  | _ ->
      O.e_sequence expression to_add

let repair_mutable_variable_in_matching (match_body : O.expression)
    (element_names : O.expression_variable list) (env : I.expression_variable)
    =
  let%bind ((dv, fv), mb) =
...
and repair_mutable_variable_in_loops (for_body : O.expression)
    (element_names : O.expression_variable list) (env : O.expression_variable)
    =
  let%bind ((dv, fv), fb) =
    Self_ast_sugar.fold_map_expression
      (* TODO : these should use Variables sets *)
...
and store_mutable_variable (free_vars : I.expression_variable list) =
  if List.length free_vars == 0 then O.e_unit ()
  else
    let aux var = (Var.to_name var, O.e_variable var) in
    O.e_record_ez (List.map aux free_vars)

and restore_mutable_variable (expr : O.expression -> O.expression_content)
    (free_vars : O.expression_variable list) (env : O.expression_variable) =
...
let rec compile_type_expression : I.type_expression -> O.type_expression result
 ...
 fun te ->
  ...
  | I.T_variable type_variable ->
      return @@ T_variable type_variable
  | I.T_constant type_constant ->
      return @@ T_constant type_constant
...
and compile_assign {variable; access_path; expression} expr =
  let accessor ?loc s a =
    match a with
    | I.Access_tuple _i ->
        failwith "adding tuple soon"
    | I.Access_record a ->
        ok @@ O.e_record_accessor ?loc s a
    | I.Access_map k ->
  ...
  let update ?loc (s : O.expression) a e =
    ...
    | I.Access_tuple _i ->
        failwith "adding tuple soon"
    | I.Access_record a ->
        ok @@ O.e_record_update ?loc s a e
    | I.Access_map k ->
        let%bind k = compile_expression k in
...
let rec uncompile_type_expression :
 ...
 fun te ->
  ...
  | O.T_arrow {type1; type2} ->
      ...
      return @@ T_arrow {type1; type2}
  | O.T_variable type_variable ->
      return @@ T_variable type_variable
  | O.T_constant type_constant ->
      return @@ T_constant type_constant

 ...
 fun te ->
  ...
  | I.T_tuple tuple ->
      let aux (i, acc) el =
        ...
        ok @@ (i + 1, (O.Label (string_of_int i), el) :: acc)
      in
...
let rec compile_expression : I.expression -> O.expression result =
 fun e ->
  ...
  | I.E_let_in {let_binder; inline; rhs; let_result} ->
      ...
      return
      @@ O.E_let_in {let_binder = (binder, ty_opt); inline; rhs; let_result}
  ...
  | I.E_look_up look_up ->
      ...
      return
      @@ O.E_constant {cons_name = C_MAP_FIND_OPT; arguments = [index; path]}
  ...
  | I.E_cond {condition; then_clause; else_clause} ->
      ...
      return
      @@ O.E_matching {matchee; cases = Match_bool {match_true; match_false}}
  | I.E_sequence {expr1; expr2} ->
      ...
      @@ O.E_let_in
           {
             let_binder = (Var.of_name "_", Some O.t_unit);
             rhs = expr1;
             let_result = expr2;
             inline = false;
           }
  | I.E_skip ->
      ok @@ O.e_unit ~loc:e.location ()
  | I.E_tuple t ->
      let aux (i, acc) el =
...
and uncompile_type_operator : O.type_operator -> I.type_operator result =
 fun t_o ->
  ...
  | TC_big_map (k, v) ->
      ...
      ok @@ I.TC_big_map (k, v)
  | TC_map_or_big_map _ ->
      failwith "TC_map_or_big_map shouldn't be uncompiled"
  | TC_arrow (i, o) ->
...
let rec uncompile_expression : O.expression -> I.expression result =
 fun e ->
  ...
  | O.E_let_in {let_binder; inline = false; rhs = expr1; let_result = expr2}
    when let_binder = (Var.of_name "_", Some O.t_unit) ->

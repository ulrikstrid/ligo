...
type abs_mapper = Expression of exp_mapper | Type_expression of ty_exp_mapper

let rec map_expression : exp_mapper -> expression -> expression result =
 fun f e ->
  ...
  | (E_literal _ | E_variable _ | E_skip) as e' ->
      return e'

and map_type_expression :
    ty_exp_mapper -> type_expression -> type_expression result =
 fun f te ->
  ...
  | T_operator _ | T_variable _ | T_constant _ ->
      ok te'

and map_cases : exp_mapper -> matching_expr -> matching_expr result =
 fun f m ->
  match m with
  | Match_bool {match_true; match_false} ->
...
and map_program : abs_mapper -> program -> program result =
 fun m p ->
...
let rec fold_map_expression :
 ...
 fun f a e ->
  ...
  if not continue then ok (init', e')
    ...
    | E_map lst ->
        let%bind (res, lst') =
          bind_fold_map_list (bind_fold_map_pair self) init' lst
        in
    ...
    | E_sequence {expr1; expr2} ->
        let%bind (res, (expr1, expr2)) =
          bind_fold_map_pair self init' (expr1, expr2)
        in

...
module Errors = struct
  ...
  let bad_empty_arity cst loc () =
    let cst_name =
      thunk @@ Format.asprintf "%a" Stage_common.PP.constant cst
    in
  ...
  let bad_single_arity cst loc () =
    ...
    let title = thunk @@ "Wrong " ^ cst_name () ^ " literal arity" in
    let message =
      thunk @@ cst_name () ^ " literal expects a single parameter"
    in
    let data =
      [("location", fun () -> Format.asprintf "%a" Location.pp loc)]
    in
  ...
  let bad_map_param_type cst loc () =
    ...
    let message =
      thunk @@ cst_name () ^ " literal expects a list of pairs as parameter"
    in
    let data =
      [("location", fun () -> Format.asprintf "%a" Location.pp loc)]
    in
  ...
  let bad_set_param_type cst loc () =
    ...
    let title = thunk @@ "Wrong " ^ cst_name () ^ " literal parameter type" in
    let message =
      thunk @@ cst_name () ^ " literal expects a list as parameter"
    in
    let data =
      [("location", fun () -> Format.asprintf "%a" Location.pp loc)]
    in
 ...
 fun e ->
  ...
  | E_literal (Literal_address s) as l ->
      ...
      let%bind (_contract : Protocol.Alpha_context.Contract.t) =
        ...
        @@ Protocol.Alpha_context.Contract.of_b58check s
      in
      return l
  | E_literal (Literal_signature s) as l ->
      let open Tezos_crypto in
      let%bind (_sig : Crypto.Signature.t) =
        Trace.trace_tzresult (bad_format e) @@ Signature.of_b58check s
      in
      return l
  ...
  | E_constant {cons_name = C_BIG_MAP_LITERAL as cst; arguments = lst} ->
      let%bind elt =
        trace_option (bad_single_arity cst e.location) @@ List.to_singleton lst
      in
      let%bind lst =
        trace_strong (bad_map_param_type cst e.location)
        @@ get_e_list elt.expression_content
      in
      let aux (e : expression) =
        trace_strong (bad_map_param_type cst e.location)
        @@ let%bind tpl = get_e_tuple e.expression_content in
           let%bind (a, b) =
             trace_option (simple_error "of pairs") @@ List.to_pair tpl
           in
           ok (a, b)
      in

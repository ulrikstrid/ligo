...
let type_expression_subst = if use_new_typer then Typer_new.type_expression_subst else Typer_old.type_expression false (* the old typer does not have unification variables that would need substitution, so no need to "subst" anything. *)

...
and type_expression' : environment -> ?tv_opt:O.type_expression -> I.expression -> (O.expression, typer_error) result = fun e ?tv_opt ae ->
  ...
  | E_application ({lamb; args} as app) -> (
    ...
    | O.E_constant {cons_name (*LIGO_STRING here for consname*) ; _} -> (
      let%bind spec_opt = Constant_special_cases.special_cases e type_expression' tv_opt ae app in
      match spec_opt with
      | Some lst' ->
        let tv_lst = List.map get_type_expression lst' in
        let%bind (_, tv) = type_constant cons_name ae.location tv_lst tv_opt in
        let aux : O.expression list -> O.expression = fun lst ->
          ignore lst ; failwith "REMITODO : get args back into a E_record (...)"
        in
        return (E_application {lamb=lamb';args = aux lst'}) tv
      | None ->
        let%bind args' = type_expression' e args in
        let aux : O.expression -> O.expression list = fun exp ->
          ignore exp ; failwith "REMITODO : get the E_record (...) list of arguments"
        in
        let tv_lst = List.map get_type_expression (aux args') in
        let%bind (_, tv) =
          type_constant cons_name ae.location tv_lst tv_opt in

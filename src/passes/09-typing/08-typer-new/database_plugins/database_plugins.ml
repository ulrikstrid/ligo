...
module MapPlugins = functor (F : MappedFunction) -> struct
  let f :
    ...
    = fun extra_args fieldsIn ->
      ...
      let%bind refined_typeclasses_back         = (let module F = F.F(RefinedTypeclassesBack)        in F.f extra_args fieldsIn#refined_typeclasses_back) ;       in
      ...
      F.Monad.return (object
        ...
        method refined_typeclasses_back         = refined_typeclasses_back

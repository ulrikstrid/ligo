...
let peephole_expression : expression -> (expression , self_ast_imperative_error) result = fun e ->
  ...
  | E_application {lamb; args} as e_const -> (
    ...
    | Some v when Var.equal v.wrap_content ev_big_map_literal.wrap_content -> (
      let%bind elt =
    ...
    | Some v when Var.equal v.wrap_content ev_map_literal.wrap_content -> (
      let%bind elt =
        trace_option (bad_single_arity v e) @@
          match lst with
          | Some lst -> List.to_singleton lst
    ...
    | Some v when Var.equal v.wrap_content ev_big_map_empty.wrap_content -> (
    ...
    | Some v when Var.equal v.wrap_content ev_map_empty.wrap_content -> (
      let%bind () = match lst with
        | Some [] -> ok ()
        | _ -> fail (bad_empty_arity v e)
      in
    ...
    | Some v when Var.equal v.wrap_content ev_set_literal.wrap_content -> (
      let%bind elt =
        trace_option (bad_single_arity v e) @@
          match lst with
          | Some lst -> List.to_singleton lst
    ...
    | Some v when Var.equal v.wrap_content ev_set_empty.wrap_content -> (
      let%bind () = match lst with
        | Some [] -> ok ()
        | _ -> fail (bad_empty_arity v e)
      in

...
let rec fold_expression : ('a, 'err) folder -> 'a -> expression -> ('a, 'err) result = fun f init e ->
  ...
  | E_literal _ | E_variable _ | E_raw_code _ | E_skip | E_import _ -> ok init' (* TODO: consider moving import *)
...
let rec map_expression : 'err exp_mapper -> expression -> (expression, 'err) result = fun f e ->
  ...
  | E_import _ | E_literal _ | E_variable _ | E_raw_code _ | E_skip as e' -> return e'

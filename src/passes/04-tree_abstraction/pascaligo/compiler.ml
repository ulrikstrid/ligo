...
and conv : CST.pattern -> (AST.ty_expr AST.pattern,_) result =
  ...
  | CST.PVar var ->
    ...
    let b =
      ...
      { var ; ascr = None ; attributes = Stage_common.Helpers.default_binder_attributes }
    in
...
and compile_data_declaration : next:AST.expression -> CST.data_decl -> _ =
  ...
  | LocalFun fun_decl ->
      ...
      return loc fun_var fun_type Stage_common.Helpers.default_binder_attributes attr lambda
...
and compile_fun_decl : CST.fun_decl -> (string * expression_variable * type_expression option * AST.attributes * expression , _) Trace.result =
  ...
  let (lambda, fun_type) =
    ...
    | lst ->
        ...
        let lambda : _ AST.lambda = {
          binder={var=binder;ascr=input_type;attributes=Stage_common.Helpers.default_binder_attributes};
          output_type = ret_type;
...
and compile_declaration : CST.declaration -> _ result =
  ...
  | FunDecl {value;region} ->
    ...
    let binder = {var;ascr;attributes=Stage_common.Helpers.default_binder_attributes} in

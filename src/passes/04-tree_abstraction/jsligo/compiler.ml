  ...
  | TApp app ->
    ...
    (match operator.value with
      ...
      | "michelson_pair" ->
        ...
        | _ -> fail @@ michelson_type_wrong_arity loc operator.value)
      | "sapling_state" ->
        failwith "TODO: sapling state is not supported in jsligo yet"
      (*   let lst = npseq_to_list args.value.inside in
       *   (match lst with
       *   | [(a : CST.type_expr)] -> (
...
let rec compile_expression : CST.expr -> AST.expr result = fun e ->
...
and compile_let_to_declaration : CST.let_ -> AST.declaration result = fun _let_ ->
  failwith "TODO"

and compile_statement_to_expression : CST.statement -> AST.expression result = fun _statement ->
  failwith "TODO"

and compile_statements_to_expression : CST.statements -> AST.expression result = fun _statements ->
  failwith "TODO"

and compile_statement_to_declaration : CST.statement -> AST.declaration result = fun _statement ->
  failwith "TODO"

and compile_statements_to_program : CST.ast -> AST.program result = fun _ast ->
  failwith "TODO"

and compile_program : CST.ast -> AST.program result = fun t ->
  ...
  in
  let%bind lst = bind_map_list aux @@ npseq_to_list t.statements in
  ok @@ lst

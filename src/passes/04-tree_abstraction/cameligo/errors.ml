...
type abs_error = [
  | `Concrete_cameligo_recursive_fun of ExtRegion.t
  ...
  | `Concrete_cameligo_funarg_tuple_type_mismatch of ExtRegion.t * Raw.pattern * Raw.type_expr
  | `Concrete_cameligo_unsupported_deep_pattern_matching of ExtRegion.t
  ]
...
let error_ppformat : display_format:string display_format ->
  ...
  | Human_readable | Dev -> (
    ...
    | `Concrete_cameligo_unsupported_pattern_type pl ->
      Format.fprintf f
        ...
        Snippet.pp_lift (List.fold_left (fun a p -> (Region.cover a (Raw.pattern_to_region p).t_region)) Region.ghost pl)
    | `Concrete_cameligo_unsupported_string_singleton te ->
      Format.fprintf f
        "@[<hv>%a@.Invalid type. @.It's not possible to assign a string to a type. @]"
        Snippet.pp_lift (Raw.type_expr_to_region te).t_region
  ...
  * a cons list pattern \"head :: tail\".@]"
        Snippet.pp_lift (Raw.pattern_to_region cons).t_region
    ...
    | `Concrete_cameligo_michelson_type_wrong (texpr,name) ->
      ...
      "@[<hv>%a@.Invalid \"%s\" type.@.At this point, an annotation, in the form of a string, is expected for the preceding type. @]"
         Snippet.pp_lift (Raw.type_expr_to_region texpr).t_region
         name
    ...
    | `Concrete_cameligo_missing_funarg_annotation v ->
      Format.fprintf f
        "@[<hv>%a@.Missing a type annotation for argument \"%s\". @]"
          Snippet.pp_lift v.region.t_region
          v.value
    | `Concrete_cameligo_funarg_tuple_type_mismatch (region, pattern, texpr) -> (
      ...
      Format.fprintf
        ...
        Snippet.pp_lift region.t_region
        p
        t
    )
  )
...
let error_jsonformat : abs_error -> Yojson.Safe.t = fun a ->
  ...
  | `Concrete_cameligo_unsupported_deep_pattern_matching l ->
    ...
    let content = `Assoc [
      ("message", message );
      ("location", Location.to_yojson (Snippet.lift l.t_region));] in
    json_error ~stage ~content
  | `Concrete_cameligo_recursive_fun reg ->
    let message = `String "Untyped recursive functions are not supported yet" in
    let loc = Format.asprintf "%a" Location.pp_lift reg.t_region in
    let content = `Assoc [
      ("message", message );
  ...
  | `Concrete_cameligo_unsupported_pattern_type pl ->
    let loc = Format.asprintf "%a"
      Location.pp_lift (List.fold_left (fun a p -> Region.cover a (Raw.pattern_to_region p).t_region) Region.ghost pl) in
  ...
  | `Concrete_cameligo_unsupported_string_singleton te ->
    ...
    let loc = Format.asprintf "%a" Location.pp_lift (Raw.type_expr_to_region te).t_region in
    let content = `Assoc [
      ("message", message );
      ("location", `String loc);] in
    json_error ~stage ~content
  ...
  | `Concrete_cameligo_unsupported_deep_list_pattern cons ->
    ...
    let loc = Format.asprintf "%a" Location.pp_lift @@ (Raw.pattern_to_region cons).t_region in
    let content = `Assoc [
      ("message", message );
      ("location", `String loc);] in
    json_error ~stage ~content
  ...
  | `Concrete_cameligo_michelson_type_wrong (texpr,name) ->
    ...
    let loc = Format.asprintf "%a" Location.pp_lift (Raw.type_expr_to_region texpr).t_region in
    let content = `Assoc [
      ("message", `String message );
      ("location", `String loc); ] in
    json_error ~stage ~content
  ...
  | `Concrete_cameligo_missing_funarg_annotation v ->
    ...
    let loc = Format.asprintf "%a" Location.pp_lift v.region.t_region in
    let content = `Assoc [
      ("message", `String message );
      ("location", `String loc); ] in
    json_error ~stage ~content
  | `Concrete_cameligo_funarg_tuple_type_mismatch (r, _, _) ->
    ...
    let loc = Format.asprintf "%a" Location.pp_lift r.t_region in
    let content = `Assoc [
      ("message", `String message );
      ("location", `String loc);
    ] in

...
and pp_patt_c_app {value; _} =
  ...
  | ctor, Some (PVar _ as pat) ->
      prefix 2 1 (pp_ident ctor)  (pp_pattern pat)
  | ctor, Some (_ as pat)->
      prefix 2 0 (pp_ident ctor)  (pp_pattern pat)
...
and pp_cons {value; _} =
  ...
  string "[" ^^ (pp_pattern lpattern ^^ string ", ")
  ^^ group (break 0 ^^ string "..." ^^ pp_pattern rpattern) ^^ string "]"
...
and pp_cond_expr {value; _} =
  ...
  | Some (_,{inside=(ifnot,_);_}) ->
    if_then
    ^^ string " else"
    ^^ string " {" ^^ break 0 ^^ group (nest 2 (break 2 ^^ pp_expr ifnot))
    ^^ hardline ^^ string "}"
...
and pp_fun {value; _} =
  ...
  and annot   =
    match lhs_type with
      None -> empty
    | Some (_, e) ->
        group (break 0 ^^ string ": " ^^ nest 2 (pp_type_expr e)) in
  match body with
...
and pp_cartesian {value; _} =
  ...
  in string "(" ^^ nest 1 (pp_type_expr head
                           ^^ (if tail <> [] then string "," else empty)
                           ^^ app (List.map snd tail)) ^^ string ")"

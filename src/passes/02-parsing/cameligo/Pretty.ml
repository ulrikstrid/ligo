...
| PCtorApp a -> pp_region_reg pp_pctor_app a
...
and pp_module_decl decl =
  let {kwd_module; name; eq; kwd_struct; declaratons; kwd_end} = decl.value in
...
and pp_module_alias decl =
  let {kwd_module; alias; eq; mod_path} = decl.value in
  pp_region_t (string "module ") kwd_module ^^ pp_region_reg pp_ident alias ^^ pp_region_t (string " =") eq
  ^^ group (nest 0 (break 1 ^^ pp_nsepseq "." (pp_region_reg pp_ident) mod_path))
...
| ECtorApp a -> pp_region_reg pp_ctor_app a
...
and pp_module_access :
  ...
  group (pp_region_reg pp_ident module_name ^^ string "."
        ^^ break 0 ^^ print field)
...
and pp_mod_in {value; _} =
  ...
  let {kwd_module; name; eq; kwd_struct; structure; kwd_end} = mod_decl
...
and pp_variant {value; _} =
  ...
  let pre = if attr = [] then pp_region_reg pp_ident ctor
            else group (pp_attributes attr ^/^ pp_region_reg pp_ident ctor) in
  match arg with
    None -> pre
...
and pp_type_app {value=ctor, tuple; _} =
  pp_type_tuple tuple ^^ pp_region_t (group (nest 2 (break 1 ^^ pp_type_ctor ctor))) ctor.region

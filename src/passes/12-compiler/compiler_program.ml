...
module Errors = struct
  ...
  let contract_entrypoint_must_be_literal ~loc =
    ...
    let content () =
      "For get_entrypoint, entrypoint must be given as a literal string"
    in
    let data = [("location", fun () -> loc)] in
    error ~data title content
end
...
and translate_expression (expr : expression) (env : environment) :
  ...
  | E_application (f, arg) ->
      ...
      @@ let%bind f = translate_expression f env in
         ...
         return @@ seq [arg; dip f; prim I_EXEC]
  | E_variable x ->
  ...
  | E_if_bool (c, a, b) ->
      ...
      let%bind a' = translate_expression a env in
      let%bind b' = translate_expression b env in
      let%bind code = ok (seq [c'; i_if a' b']) in
      return code
  | E_if_none (c, n, (ntv, s)) ->
      let%bind c' = translate_expression c env in
      ...
      let%bind code =
        ok
          (seq
             [ c';
               i_if_left
                 (seq [l'; i_comment "restrict left"; dip i_drop])
                 (seq [r'; i_comment "restrict right"; dip i_drop]) ])
      in
      return code
  | E_let_in (v, _, expr, body) ->
      ...
      let%bind code =
        ok (seq [expr'; body'; i_comment "restrict let"; dip i_drop])
      in
      return code
  ...
  | E_fold ((v, body), collection, initial) ->
      ...
      let code =
        seq
          [ collection';
            dip initial';
            i_iter (seq [i_swap; i_pair; body'; dip i_drop]) ]
      in
      ok code
  ...
  | E_while (expr, block) ->
      ...
      @@ seq
           [ expr';
             prim ~children:[seq [block'; i_drop; expr']] I_LOOP;
             i_push_unit ]

and translate_function_body ({body; binder} : anon_function) lst input :
    michelson result =
...
and translate_function anon env input_ty output_ty : michelson result =
  ...
  let%bind lambda_ty =
    Compiler_type.lambda_closure (small_env, input_ty, output_ty)
  in
  let%bind lambda_body_code =
  ...
  | _ :: _ ->
      ...
      let%bind closure_pack_code =
        Compiler_environment.pack_closure env selector
      in
      ok
      @@ seq
           [ closure_pack_code;

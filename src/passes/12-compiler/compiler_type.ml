...
module Ty = struct
  ...
  let pair a b =
    Pair_t
      ((a, None, None), (b, None, None), None, has_big_map a || has_big_map b)

  let union a b =
    Union_t ((a, None), (b, None), None, has_big_map a || has_big_map b)
  ...
  let pair_ann (anna, a) (annb, b) =
    Pair_t
      ( (a, field_annot anna, None),
        (b, field_annot annb, None),
        None,
        has_big_map a || has_big_map b )

  let not_comparable name () =
    error (thunk "not a comparable type") (fun () -> name) ()

  let not_compilable_type name () =
    error (thunk "not a compilable type") (fun () -> name) ()

  let comparable_type_base : type_constant -> ex_comparable_ty result =
   fun tb ->
    let return x = ok @@ Ex_comparable_ty x in
    match tb with
    | TC_unit ->
        fail (not_comparable "unit")
    | TC_void ->
        fail (not_comparable "void")
    | TC_bool ->
        return bool_k
    | TC_nat ->
        return nat_k
    ...
    | TC_bytes ->
        return bytes_k
    | TC_operation ->
        fail (not_comparable "operation")
    | TC_signature ->
        fail (not_comparable "signature")
    | TC_key ->
        fail (not_comparable "key")
    | TC_key_hash ->
        return key_hash_k
    | TC_chain_id ->
        fail (not_comparable "chain_id")

  let comparable_leaf :
      type a. (a, _) comparable_struct -> (a, leaf) comparable_struct result =
   fun a ->
    match a with
    | Pair_key _ ->
        fail (not_comparable "pair (use (a,(b,c)) instead of (a,b,c))")
    | Int_key annot ->
        ok (Int_key annot)
    | Nat_key annot ->
        ok (Nat_key annot)
    ...
    | Bool_key annot ->
        ok (Bool_key annot)
    | Key_hash_key annot ->
        ok (Key_hash_key annot)
    | Timestamp_key annot ->
        ok (Timestamp_key annot)
    | Address_key annot ->
        ok (Address_key annot)

  let rec comparable_type : type_value -> ex_comparable_ty result =
   fun tv ->
    match tv with
    | T_base b ->
        comparable_type_base b
    | T_function _ ->
        fail (not_comparable "function")
    | T_or _ ->
        fail (not_comparable "or")
    | T_pair ((_, a), (_, b)) ->
    ...
    | T_map _ ->
        fail (not_comparable "map")
    | T_big_map _ ->
        fail (not_comparable "big_map")
    | T_list _ ->
        fail (not_comparable "list")
    | T_set _ ->
        fail (not_comparable "set")
    | T_option _ ->
        fail (not_comparable "option")
    | T_contract _ ->
        fail (not_comparable "contract")

  let base_type : type_constant -> ex_ty result =
   fun b ->
  ...
  let rec type_ : type_value -> ex_ty result = function
    | T_base b ->
        base_type b
    | T_pair (t, t') ->
        annotated t
        >>? fun (ann, Ex_ty t) ->
        annotated t'
        >>? fun (ann', Ex_ty t') -> ok @@ Ex_ty (pair_ann (ann, t) (ann', t'))
    | T_or (t, t') ->
        annotated t
        >>? fun (ann, Ex_ty t) ->
        annotated t'
        >>? fun (ann', Ex_ty t') -> ok @@ Ex_ty (union_ann (ann, t) (ann', t'))
    | T_function (arg, ret) ->
  ...
  and environment_representation e =
  ...
  and environment : environment -> ex_stack_ty result =
   fun env ->
    let%bind lst = bind_map_list type_ @@ List.map snd env in
...
let base_type : type_constant -> O.michelson result = function
  | TC_unit ->
      ok @@ O.prim T_unit
  | TC_void ->
      fail (Ty.not_compilable_type "void")
  | TC_bool ->
      ok @@ O.prim T_bool
  | TC_int ->
      ok @@ O.prim T_int
  ...
  | TC_address ->
      ok @@ O.prim T_address
  | TC_timestamp ->
      ok @@ O.prim T_timestamp
  | TC_bytes ->
      ok @@ O.prim T_bytes
  | TC_operation ->
      ok @@ O.prim T_operation
  | TC_signature ->
      ok @@ O.prim T_signature
  | TC_key ->
      ok @@ O.prim T_key
  | TC_key_hash ->
      ok @@ O.prim T_key_hash
  | TC_chain_id ->
      ok @@ O.prim T_chain_id

let rec type_ : type_value -> O.michelson result = function
  | T_base b ->
      base_type b
  | T_pair (t, t') ->
      annotated t
      >>? fun t ->
      annotated t' >>? fun t' -> ok @@ O.prim ~children:[t; t'] O.T_pair
  | T_or (t, t') ->
      annotated t
      >>? fun t ->
      annotated t' >>? fun t' -> ok @@ O.prim ~children:[t; t'] O.T_or
  | T_map kv ->
...
and annotated : type_value annotated -> O.michelson result = function
  | (Some ann, o) ->
      let%bind o' = type_ o in
      ok (O.annotate ("%" ^ ann) o')
  | (None, o) ->
...
and environment env = bind_map_list type_ @@ List.map snd env

and lambda_closure (c, arg, ret) =
  let%bind arg = type_ arg in
  let%bind ret = type_ ret in
  match c with
  | [] ->
...
and environment_closure = function
  | [] ->

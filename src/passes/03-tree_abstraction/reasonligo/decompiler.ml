...
let rec decompile_expression : AST.expression -> _ result = fun expr ->
  ...
  | E_set set ->
    ...
    return_expr @@ CST.ECall (wrap @@ (var,args))
  | E_import _ ->
    (* let hd, tl = import.path in
     * let path = (wrap hd), List.map (fun e -> rg, wrap e) tl in
     * return_expr @@ CST.EImport (wrap path) *)
    failwith @@ Format.asprintf "import expressions NYI in reasonligo"

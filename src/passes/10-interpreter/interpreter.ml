...
let apply_comparison : Ast_typed.constant' -> value list -> value result =
 fun c operands ->
  ...
  | (comp, [V_Ct (C_timestamp a'); V_Ct (C_timestamp b')]) ->
      let f_op =
        ...
        | C_GE -> (
            >= )
        | _ ->
            failwith "apply compare must be called with a comparative constant"
      in
      ok @@ v_bool (f_op a' b')
  ...
  | _ ->
      let () =
        List.iter
          (fun el -> Format.printf "%s" (Ligo_interpreter.PP.pp_value el))
          operands
      in
...
let rec apply_operator : Ast_typed.constant' -> value list -> value result =
 fun c operands ->
  ...
  (*TODO This raise is here until we properly implement effects*)
  | (C_SIZE, [(V_Set l | V_List l)]) ->
      return_ct @@ C_nat (List.length l)
  | (C_SIZE, [V_Map l]) ->
      return_ct @@ C_nat (List.length l)
  | (C_SIZE, [V_Ct (C_string s)]) ->
      return_ct @@ C_nat (String.length s)
  | (C_SIZE, [V_Ct (C_bytes b)]) ->
      return_ct @@ C_nat (Bytes.length b)
  | (C_NOT, [V_Ct (C_bool a')]) ->
      return_ct @@ C_bool (not a')
  | (C_INT, [V_Ct (C_nat a')]) ->
      return_ct @@ C_int a'
  | (C_ABS, [V_Ct (C_int a')]) ->
      return_ct @@ C_int (abs a')
  | (C_NEG, [V_Ct (C_int a')]) ->
      return_ct @@ C_int (-a')
  | (C_SOME, [v]) ->
      return_some v
  | (C_IS_NAT, [V_Ct (C_int a')]) ->
      if a' > 0 then return_some @@ V_Ct (C_nat a') else return_none ()
  | (C_FOLD_CONTINUE, [v]) ->
      ok @@ v_pair (v_bool true, v)
  | (C_FOLD_STOP, [v]) ->
      ok @@ v_pair (v_bool false, v)
  | (C_ASSERTION, [v]) ->
  ...
  | (C_MAP_FIND, [k; V_Map l]) -> (
    ...
    | Some v ->
        ok @@ v
    | None ->
        raise (Temporary_hack "failed map find") )
  (* binary *)
  | ((C_EQ | C_NEQ | C_LT | C_LE | C_GT | C_GE), _) ->
      apply_comparison c operands
  ...
  | (C_MUL, [V_Ct (C_nat a'); V_Ct (C_mutez b')]) ->
      return_ct @@ C_mutez (a' * b')
  | (C_MUL, [V_Ct (C_mutez a'); V_Ct (C_mutez b')]) ->
      return_ct @@ C_mutez (a' * b')
  ...
  | (C_CONCAT, [V_Ct (C_string a'); V_Ct (C_string b')]) ->
      return_ct @@ C_string (a' ^ b')
  | (C_CONCAT, [V_Ct (C_bytes a'); V_Ct (C_bytes b')]) ->
      return_ct @@ C_bytes (Bytes.cat a' b')
  | (C_OR, [V_Ct (C_bool a'); V_Ct (C_bool b')]) ->
      return_ct @@ C_bool (a' || b')
  | (C_AND, [V_Ct (C_bool a'); V_Ct (C_bool b')]) ->
      return_ct @@ C_bool (a' && b')
  | (C_XOR, [V_Ct (C_bool a'); V_Ct (C_bool b')]) ->
      return_ct @@ C_bool ((a' || b') && not (a' && b'))
  | (C_LIST_EMPTY, []) ->
      ok @@ V_List []
  ...
  | (C_FOLD_WHILE, [V_Func_val (arg_name, body, env); init]) ->
      let rec aux el =
        ...
        if continue then aux res else ok folded_val
      in
      aux @@ v_pair (v_bool true, init)
  (* tertiary *)
  | (C_SLICE, [V_Ct (C_nat st); V_Ct (C_nat ed); V_Ct (C_string s)]) ->
      generic_try (simple_error "bad slice")
      @@ fun () -> V_Ct (C_string (String.sub s st ed))
  ...
  | (C_MAP_MEM, [k; V_Map kvs]) ->
      ok @@ v_bool (List.mem_assoc k kvs)
  | (C_MAP_ADD, [k; v; (V_Map kvs as vmap)]) ->
      if List.mem_assoc k kvs then ok vmap else ok (V_Map ((k, v) :: kvs))
  | (C_MAP_REMOVE, [k; V_Map kvs]) ->
      ok @@ V_Map (List.remove_assoc k kvs)
  | (C_MAP_UPDATE, [k; V_Construct (option, v); V_Map kvs]) -> (
    match option with
    | "Some" ->
        ok @@ V_Map ((k, v) :: List.remove_assoc k kvs)
    | "None" ->
        ok @@ V_Map (List.remove_assoc k kvs)
    | _ ->
        simple_fail "update without an option" )
  | (C_SET_EMPTY, []) ->
      ok @@ V_Set []
  | (C_SET_ADD, [v; V_Set l]) ->
      ok @@ V_Set (List.sort_uniq compare (v :: l))
  ...
  | (C_SET_MEM, [v; V_Set elts]) ->
      ok @@ v_bool (List.mem v elts)
  | (C_SET_REMOVE, [v; V_Set elts]) ->
      ok @@ V_Set (List.filter (fun el -> not (el = v)) elts)
  | _ ->
      let () = Format.printf "%a\n" Stage_common.PP.constant c in
      let () =
        List.iter
          (fun e -> Format.printf "%s\n" (Ligo_interpreter.PP.pp_value e))
          operands
      in
...
and eval_literal : Ast_typed.literal -> value result = function
  ...
  | Literal_address s ->
      ok @@ V_Ct (C_address s)
  | Literal_signature s ->
      ok @@ V_Ct (C_signature s)
  | Literal_key s ->
      ok @@ V_Ct (C_key s)
  | Literal_key_hash s ->
      ok @@ V_Ct (C_key_hash s)
  | Literal_chain_id s ->
      ok @@ V_Ct (C_key_hash s)
  | Literal_operation o ->
      ok @@ V_Ct (C_operation o)
  | Literal_void ->
      simple_fail "iguess ?"
 ...
 fun term env ->
  ...
  | E_application {lamb = f; args} -> (
      ...
      | V_Func_rec (fun_name, arg_names, body, f_env) ->
          ...
          eval body f_env''
      | _ ->
          simple_fail "trying to apply on something that is not a function" )
  ...
  | E_record_accessor {record; path} -> (
      ...
      | V_Record recmap ->
          let%bind a =
            trace_option (simple_error "unknown record field")
            @@ LMap.find_opt path recmap
          in
          ok a
      | _ ->
          simple_fail "trying to access a non-record" )
  | E_record_update {record; path; update} -> (
      ...
      | V_Record recmap ->
          ...
          else simple_fail "field l does not exist in record"
      | _ ->
          simple_fail "this expression isn't a record" )
  ...
  | E_matching {matchee; cases} -> (
      ...
      | (Match_list cases, V_List (head :: tail)) ->
          ...
          let env' =
            Env.extend (Env.extend env (head_var, head)) (tail_var, V_List tail)
          in
          eval body env'
      ...
      | _ ->
          simple_fail "not yet supported case"
      (* ((ctor,name),body) *) )
...
let dummy : Ast_typed.program -> string result =
 fun prg ->
  let%bind (res, _) =
    bind_fold_list
      (fun (pp, top_env) el ->
        let (Ast_typed.Declaration_constant (exp_name, exp, _, _)) =
          Location.unwrap el
        in
        let%bind v =
          (*TODO This TRY-CATCH is here until we properly implement effects*)
          try eval exp top_env with Temporary_hack s -> ok @@ V_Failure s
          (*TODO This TRY-CATCH is here until we properly implement effects*)
        in
        let pp' =
          pp ^ "\n val " ^ Var.to_name exp_name ^ " = "

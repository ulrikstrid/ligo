   ...
   | comp, [V_Ct (C_int a'); V_Ct (C_int b')]
    ...
    |comp, [V_Ct (C_timestamp a'); V_Ct (C_timestamp b')] ->
       let f_op =
         ...
         | _     ->
             failwith
               "apply compare must be called with a comparative constant"
       in
       ok @@ v_bool (f_op a' b')
 ...
 fun c operands ->
   ...
   | C_ASSERTION, [v] ->
       ...
       if pass then
         ...
         raise (Temporary_hack "failed assertion")
...
and eval : Ast_typed.expression -> env -> value result =
 fun term env ->
   ...
   | E_application {lamb= f; args} -> (
       ...
       | _                                             -> simple_fail
                                                            "trying to apply \
                                                             ...
                                                             that is not a \
                                                             function" )
   | E_lambda {binder; result} -> ok @@ V_Func_val (binder, result, env)
   ...
   | E_variable var -> Env.lookup env var
   | E_record recmap ->
       let%bind lv' =
         bind_map_list
   ...
   | E_matching {matchee; cases} -> (
       ...
       match (cases, e') with
       | Match_list cases, V_List [] -> eval cases.match_nil env
       | Match_list cases, V_List (head :: tail) ->
           let head_var, tail_var, body, _ = cases.match_cons in
           let env' =
             Env.extend
               (Env.extend env (head_var, head))
               (tail_var, V_List tail)
           in
           eval body env'
       | Match_variant (case_list, _), V_Construct (matched_c, proj) ->
           ...
           eval body env'
       | Match_bool cases, V_Ct (C_bool true) -> eval cases.match_true env
       | Match_bool cases, V_Ct (C_bool false) -> eval cases.match_false env

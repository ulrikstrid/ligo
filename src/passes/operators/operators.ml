...
module Typer = struct
  module Operator_errors = struct
    ...
    let typeclass_error msg f expected_types actual_types () =
      let message () =
        Format.asprintf
          "Expected arguments with one of the following combinations of \
           types: %a but got this combination instead: %a"
          (list_sep
             (print_f_args f Ast_typed.PP.type_expression)
             (const " or "))
          expected_types
          (print_f_args f Ast_typed.PP.type_expression)
          actual_types
  ...
  *)
  ...
  module Operators_types = struct
    ...
    let tc_timargs a b c =
      tc [a; b; c] [[nat; nat; nat]; [int; int; int] (*TODO…*)]
    ...
    let t_sub =
      forall3_tc "a" "b" "c"
      @@ fun a b c -> [tc_subarg a b c] => tuple2 a b --> c
    ...
    let t_map_remove =
      forall2 "src" "dst"
      @@ fun src dst -> tuple2 src (map src dst) --> map src dst

    let t_map_add =
      forall2 "src" "dst"
      @@ fun src dst -> tuple3 src dst (map src dst) --> map src dst

    let t_map_update =
      forall2 "src" "dst"
      @@ fun src dst -> tuple3 src (option dst) (map src dst) --> map src dst

    let t_map_mem =
      forall2 "src" "dst" @@ fun src dst -> tuple2 src (map src dst) --> bool

    let t_map_find =
      forall2 "src" "dst" @@ fun src dst -> tuple2 src (map src dst) --> dst

    let t_map_find_opt =
      forall2 "src" "dst"
      @@ fun src dst -> tuple2 src (map src dst) --> option dst

    let t_map_fold =
      forall3 "src" "dst" "acc"
      @@ fun src dst acc ->
      tuple3 ((src * dst * acc) --> acc) (map src dst) acc --> acc

    let t_map_map =
      forall3 "k" "v" "result"
      @@ fun k v result ->
      tuple2 ((k * v) --> result) (map k v) --> map k result

    (* TODO: the type of map_map_fold might be wrong, check it. *)
    let t_map_map_fold =
      forall4 "k" "v" "acc" "dst"
      @@ fun k v acc dst ->
      tuple3 ((k * v * acc) --> (acc * dst)) (map k v) (k * v)
      --> (map k dst * acc)

    let t_map_iter =
      forall2 "k" "v"
      @@ fun k v -> tuple2 ((k * v) --> unit) (map k v) --> unit

    let t_size = forall_tc "c" @@ fun c -> [tc_sizearg c] => tuple1 c --> nat
    ...
    let t_bytes_pack =
      forall_tc "a" @@ fun a -> [tc_packable a] => tuple1 a --> bytes

    (* TYPECLASS *)

    let t_bytes_unpack =
      forall_tc "a" @@ fun a -> [tc_packable a] => tuple1 bytes --> a
    ...
    let t_transaction =
      forall "a" @@ fun a -> tuple3 a mutez (contract a) --> operation
    ...
    let t_times =
      forall3_tc "a" "b" "c"
      @@ fun a b c -> [tc_timargs a b c] => tuple2 a b --> c

    (* TYPECLASS *)

    let t_div =
      forall3_tc "a" "b" "c"
      @@ fun a b c -> [tc_divargs a b c] => tuple2 a b --> c

    (* TYPECLASS *)

    let t_mod =
      forall3_tc "a" "b" "c"
      @@ fun a b c -> [tc_modargs a b c] => tuple2 a b --> c

    (* TYPECLASS *)

    let t_add =
      forall3_tc "a" "b" "c"
      @@ fun a b c -> [tc_addargs a b c] => tuple2 a b --> c
    ...
    let t_fold_while =
      forall "a" @@ fun a -> tuple2 (a --> pair bool a) a --> a
    ...
    let t_comp =
      forall_tc "a" @@ fun a -> [tc_comparable a] => tuple2 a a --> bool

    let t_concat =
      forall_tc "a" @@ fun a -> [tc_concatable a] => tuple2 a a --> a

    let t_set_empty =
      forall_tc "a" @@ fun a -> [tc_comparable a] => tuple0 --> set a

    let t_set_iter =
      forall_tc "a"
      @@ fun a -> [tc_comparable a] => tuple2 (a --> unit) (set a) --> unit

    (* TODO: check that the implementation has this type *)
    let t_set_fold =
      forall2_tc "a" "b"
      @@ fun a b ->
      [tc_comparable b] => tuple3 (pair a b --> a) (set b) a --> a

    let t_list_iter =
      forall "a" @@ fun a -> tuple2 (a --> unit) (list a) --> unit

    let t_list_map =
      forall "a" @@ fun a -> tuple2 (a --> a) (list a) --> list a

    (* TODO: check that the implementation has this type *)
    let t_list_fold =
      forall2 "a" "b" @@ fun a b -> tuple3 (pair a b --> a) (list b) a --> a

    let t_self_address = tuple0 --> address

    let t_implicit_account =
      forall_tc "a"
      @@ fun a -> [tc_storable a] => tuple1 key_hash --> contract a
  ...
  let none =
    typer_0 "NONE"
    @@ fun tv_opt ->
    match tv_opt with None -> simple_fail "untyped NONE" | Some t -> ok t
  ...
  let failwith_ =
    ...
    @@ fun t opt ->
    let%bind () = Assert.assert_true @@ is_t_string t in
  ...
  let bytes_pack : typer = typer_1 "PACK" @@ fun _t -> ok @@ t_bytes ()

  let bytes_unpack =
    typer_1_opt "UNPACK"
    @@ fun input output_opt ->
    let%bind () = assert_t_bytes input in
    trace_option (simple_error "untyped UNPACK") @@ output_opt
  ...
  let self_address = typer_0 "SELF_ADDRESS" @@ fun _ -> ok @@ t_address ()

  let self =
    typer_1_opt "SELF"
    @@ fun entrypoint_as_string tv_opt ->
    let%bind () = assert_t_string entrypoint_as_string in
    match tv_opt with None -> simple_fail "untyped SELF" | Some t -> ok @@ t

  let implicit_account =
    typer_1 "IMPLICIT_ACCOUNT"
    @@ fun key_hash ->
  ...
  let get_contract_opt =
    ...
    if not (type_expression_eq (addr_tv, t_address ())) then
      ...
      let%bind tv =
        trace_strong
          (simple_error "get_entrypoint_opt has a not-option annotation")
        @@ get_t_option tv
      in
      let%bind tv' =
        trace_strong
  ...
  let get_entrypoint =
    ...
    @@ fun entry_tv addr_tv tv_opt ->
    if not (type_expression_eq (entry_tv, t_string ())) then
      fail
      @@ simple_error
           (Format.asprintf
  ...
  let neg =
    ...
    @@ fun t ->
    let%bind () =
      Assert.assert_true (eq_1 t (t_nat ()) || eq_1 t (t_int ()))
    in
    ok @@ t_int ()
  ...
  let continue =
    typer_1 "CONTINUE" @@ fun arg -> ok @@ t_pair (t_bool ()) arg ()

  let stop = typer_1 "STOP" @@ fun arg -> ok (t_pair (t_bool ()) arg ())

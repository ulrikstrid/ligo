...
let arity : prim -> int option = function
  ...
  | I_APPLY -> Some 2
  (* not instructions *)
  | K_parameter | K_storage | K_code | D_False | D_Elt | D_Left | D_None
   |D_Pair | D_Right | D_Some | D_True | D_Unit | T_bool | T_contract | T_int
   |T_key | T_key_hash | T_lambda | T_list | T_map | T_big_map | T_nat
   |T_option | T_or | T_pair | T_set | T_signature | T_string | T_bytes
   |T_mutez | T_timestamp | T_unit | T_operation | T_address | T_chain_id ->
      None

let is_nullary_op (p : prim) : bool =
  match arity p with Some 0 -> true | _ -> false

let is_unary_op (p : prim) : bool =
  match arity p with Some 1 -> true | _ -> false

let is_binary_op (p : prim) : bool =
  match arity p with Some 2 -> true | _ -> false

let is_ternary_op (p : prim) : bool =
  match arity p with Some 3 -> true | _ -> false
...
let rec use_lambda_instr : michelson -> michelson =
 fun x ->
   ...
   | Prim (_, I_PUSH, _, _)                                                                           -> x
                                                                       (* possibly missing some nested lambdas *)
   | Prim (l, p, args, annot)                                                                         ->
...
let rec flatten_seqs : michelson -> michelson =
 fun x ->
   ...
   | Seq (l, args)            ->
       let args =
         List.concat @@ List.map (fun x -> unseq (flatten_seqs x)) args
       in
       Seq (l, args)
...
type peep4 =
  michelson * michelson * michelson * michelson -> michelson list option
...
let rec peep1 (f : peep1) : michelson list -> bool * michelson list = function
  ...
  | x1 :: xs -> (
    match f x1 with
    | Some xs' ->
        let _, xs' = peep1 f (xs' @ xs) in
        (true, xs')
    | None     ->
        let changed, xs = peep1 f xs in
        (changed, x1 :: xs) )
...
let rec peep2 (f : peep2) : michelson list -> bool * michelson list = function
  ...
  | x1 :: x2 :: xs -> (
    match f (x1, x2) with
    | Some xs' ->
        let _, xs' = peep2 f (xs' @ xs) in
        (true, xs')
    | None     ->
        let changed, xs' = peep2 f (x2 :: xs) in
        (changed, x1 :: xs') )
...
let rec peep3 (f : peep3) : michelson list -> bool * michelson list = function
  ...
  | x1 :: x2 :: x3 :: xs -> (
    ...
    | Some xs' ->
        let _, xs' = peep3 f (xs' @ xs) in
        (true, xs')
    | None     ->
        let changed, xs' = peep3 f (x2 :: x3 :: xs) in
        (changed, x1 :: xs') )
...
let rec peep4 (f : peep4) : michelson list -> bool * michelson list = function
  ...
  | x1 :: x2 :: x3 :: x4 :: xs -> (
    ...
    | Some xs' ->
        let _, xs' = peep4 f (xs' @ xs) in
        (true, xs')
    | None     ->
        let changed, xs' = peep4 f (x2 :: x3 :: x4 :: xs) in
        (changed, x1 :: xs') )

(* apply f to all seqs *)
let rec peephole (f : michelson list -> bool * michelson list) :
    michelson -> bool * michelson =
...
let rec sequence_optimizers (fs : (michelson -> bool * michelson) list) :
    michelson -> bool * michelson =
  match fs with
  | []         -> fun x -> (false, x)
  | f :: fs ->
...
let rec iterate_optimizer (f : michelson -> bool * michelson) :
    michelson -> michelson =
 fun x ->
   let changed, x = f x in
   if changed then
     iterate_optimizer f x
   else
     x
...
let opt_drop2 : peep2 = function
  ...
  (* unary_op ; DROP  ↦  DROP *)
  | Prim (_, p, _, _), Prim (_, I_DROP, [], _)     when is_unary_op p ->
      Some [i_drop]
  (* binary_op ; DROP  ↦  DROP ; DROP *)
  | Prim (_, p, _, _), Prim (_, I_DROP, [], _)     when is_binary_op p ->
      Some [i_drop; i_drop]
  (* ternary_op ; DROP  ↦  DROP ; DROP ; DROP *)
  | Prim (_, p, _, _), Prim (_, I_DROP, [], _)     when is_ternary_op p ->
      Some [i_drop; i_drop; i_drop]
  | _                                                  -> None
...
let opt_dip1 : peep1 = function
  ...
  | Prim (_, I_DIP, [Seq (_, [(Prim (_, p, _, _) as push)])], _)
    when is_nullary_op p ->
      Some [push; i_swap]
  (* DIP { unary_op }  ↦  SWAP ; unary_op ; SWAP *)
  | Prim (_, I_DIP, [Seq (_, [(Prim (_, p, _, _) as unary_op)])], _)
    when is_unary_op p ->
...
let opt_dip2 : peep2 = function
  ...
  | (Prim (_, p, _, _) as nullary_op), Prim (_, I_DIP, [Seq (_, code)], _)
    when is_nullary_op p ->
      Some (code @ [nullary_op])
  (* DIP { code } ; unary_op  ↦  unary_op ; DIP { code } *)
  | (Prim (_, I_DIP, [Seq _], _) as dip), (Prim (_, p, _, _) as unary_op)
    when is_unary_op p ->
      Some [unary_op; dip]
...
let opt_dip3 : peep3 = function
  ...
  | ( Prim (_, I_DUP, _, _),
      ...
      Prim
        (_, I_DIP, [Seq (_, [(Prim (_, (I_CAR | I_CDR), _, _) as proj2)])], _)
    ) ->
...
let opt_swap2 : peep2 = function
  (* SWAP ; SWAP  ↦  *)
  | Prim (_, I_SWAP, _, _), Prim (_, I_SWAP, _, _) -> Some []
  (* DUP ; SWAP  ↦  DUP *)
  | Prim (_, I_DUP, _, _), Prim (_, I_SWAP, _, _) -> Some [i_dup]
  (* SWAP ; ADD  ↦  ADD *)
  (* etc *)
  | ( Prim (_, I_SWAP, _, _),
      (Prim (_, (I_ADD | I_OR | I_AND | I_XOR), _, _) as comm_op) ) ->
      Some [comm_op]
  | _ -> None
...
let rec is_failing : michelson -> bool = function
  | Seq (_, []) -> false
...
let rec opt_tail_fail : michelson -> michelson = function
  | Seq (l, args)            ->
      let rec aux args =
        match args with
        | []             -> []
        | arg :: args ->
            ...
            if is_failing arg then
              [arg]
            else
              arg :: aux args
      in
      Seq (l, aux args)
  | Prim (l, p, args, annot) -> Prim (l, p, List.map opt_tail_fail args, annot)
  | x                        -> x
...
let rec opt_combine_drops (x : michelson) : michelson =
  let rec combine : michelson list -> michelson list = function
    ...
    | Prim (_, I_DROP, [], []) :: xs -> (
        ...
        | Prim (_, I_DROP, [], []) :: xs'                      ->
            Prim (-1, I_DROP, [Int (-1, Z.of_int 2)], []) :: xs'
        | Prim (_, I_DROP, [Int (_, n)], []) :: xs' ->
            Prim (-1, I_DROP, [Int (-1, Z.of_int (1 + Z.to_int n))], []) :: xs'
        | x' :: xs'                                              -> Prim
                                                                      ( -1,
                                                                        I_DROP,
                                                                        [],
  ...
  match x with
  | Seq (l, args)            -> Seq
                                  (l, combine (List.map opt_combine_drops args))
  | Prim (l, p, args, annot) ->

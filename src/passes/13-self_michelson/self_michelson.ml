...
let arity : prim -> int option = function
  ...
  | I_MUL ->
      None
  | I_SUB ->
      None (* can fail for tez *)
  (* etc *)
  | I_CONCAT ->
      None (* sometimes 1, sometimes 2 :( *)
  | I_CAST ->
      None
  | I_RENAME ->
      None
...
let rec flatten_seqs : michelson -> michelson =
 fun x ->
  ...
  | Prim (_, I_PUSH, _, _) ->
      x
  | Prim (l, p, args, annot) ->
      Prim (l, p, List.map flatten_seqs args, annot)
  | _ ->
      x
...
let opt_drop2 : peep2 = function
  (* nullary_op ; DROP  ↦  *)
  | (Prim (_, p, _, _), Prim (_, I_DROP, [], _)) when is_nullary_op p ->
      Some []
  (* DUP ; DROP  ↦  *)
  | (Prim (_, I_DUP, _, _), Prim (_, I_DROP, [], _)) ->
      Some []
  (* unary_op ; DROP  ↦  DROP *)
...
let rec opt_combine_drops (x : michelson) : michelson =
  ...
  match x with
  | Seq (l, args) ->
      Seq (l, combine (List.map opt_combine_drops args))
  | Prim (l, p, args, annot) ->

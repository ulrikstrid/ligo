...
and type_expression' : ?tv_opt:O.type_expression -> environment -> _ O'.typer_state -> I.expression -> ((environment * _ O'.typer_state * O.expression) * _, typer_error) result = fun ?tv_opt e state ae ->
  ...
  | E_module_accessor {module_name; element} ->
    ...
    let rec aux env module_name (element : I.expression) =
      ...
      | E_module_accessor {module_name;element} ->
        let%bind modules, element, var, constraints, ty' = aux module_ module_name element in
        let modules = (module_name,ty') :: modules in
        ok @@ (modules, element, var, constraints, ty)
      | E_variable var ->
        let%bind (_,_,element),constraints = self module_ state element in
        ok @@ ([], element, Var.to_name var.wrap_content,constraints,ty)
      | _ -> failwith "cornercase : module_accessor cannot be parse like that"
    in
    let%bind (modules_ty, element,var,constraints,ty) = aux e module_name element in
    ...
    return_wrapped expr' e state constraints wrapped

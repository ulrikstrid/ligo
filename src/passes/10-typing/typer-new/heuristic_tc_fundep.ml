...
let wrapper ~(original:c_typeclass_simpl) ~repr tc =
  ...
  let tc_args = List.map (fun x -> wrap (Todo "no idea") @@ P_variable (repr x)) tc.args in
  {
    reason = tc.reason_typeclass_simpl;
    c = C_typeclass {
  ...
  let ret = [
      {
        ...
        add_constraints = [wrapper ~original:selected.tc ~repr restricted];
        proof_trace = Axiom (HandWaved "cut with the following (restricted => removed_typeclass) to show that the removal does not lose info, (removed_typeclass => selected.c => restricted) to show that the cleaned vesion does not introduce unwanted constraints.")
      }
    ] in
  ok ret

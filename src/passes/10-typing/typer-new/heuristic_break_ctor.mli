...
(* module M : (module type of Typesystem.Solver_types.Heuristic_plugin.M) *)
...
(* module Flds : sig type t end = struct
 *   type 'self t = <
 *     grouped_by_variable : type_variable GroupedByVariable.t ;
 *     ..
 *   > as 'self
...
(* module A : sig
 *     type 'self t
 *   end with type 'self t = <
 *       grouped_by_variable : type_variable GroupedByVariable.t ;
 *       ..
 *     > as 'self
 * module B : sig type t end
 * module type T = Typesystem.Solver_types.Ex_heuristic_plugin(A).S with module Selector_output = B
 * module Heuristic : T *)

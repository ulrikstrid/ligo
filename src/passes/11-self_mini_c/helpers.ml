...
let rec fold_type_value :
    ('a -> type_value -> 'a result) -> 'a -> type_value -> 'a result =
 fun f init t ->
   ...
   | T_list a | T_set a | T_contract a | T_option a -> self init' a
   | T_base _ -> ok init'
...
let rec fold_expression : 'a folder -> 'a -> expression -> 'a result =
 fun f init e ->
   ...
   | E_variable _ | E_skip | E_make_none _ | E_literal _ -> ok init'
   | E_constant c ->
...
let rec map_expression : mapper -> expression -> expression result =
 fun f e ->
   ...
   match e'.content with
   | (E_variable _ | E_literal _ | E_skip | E_make_none _) as em -> return em
   | E_constant c ->
...
let map_sub_level_expression : mapper -> expression -> expression result =
 fun f e ->
   match e.content with

...
module Errors = struct
  ...
  let unbound_constructor (e : environment) (c : I.constructor')
      (loc : Location.t) () =
  ...
  let constant_declaration_error (name : I.expression_variable) (ae : I.expr)
      ...
      [ ( "constant",
        ...
        ( "expected",
          fun () ->
            match expected with
            | None ->
                "(no annotation for the expected type)"
            | Some expected ->
  ...
  let type_error ?(msg = "") ~(expected : O.type_expression)
      ~(actual : O.type_expression) ~(expression : I.expression)
      (loc : Location.t) () =
    let title = thunk "type error" in
    let message () = msg in
    let data =
        ...
        let error =
          ...
          let content () =
            Format.asprintf
              ...
              constructor
              O.Environment.PP.full_environment
              e
          in
          error title content
        in
      ...
      let%bind (wrapped, tv) =
        ...
        | T_record record -> (
            ...
            | None ->
                failwith
                @@ Format.asprintf
                     "field %a is not part of record"
                     Stage_common.PP.label
                     path )
        | _ ->
            failwith "Update an expression which is not a record"
      in
      let%bind () =
...
(* Advanced *)
and type_constant (name : I.constant') (lst : O.type_expression list)
...
let untype_type_value (t : O.type_expression) : I.type_expression result =
  ...
  | Some s ->
      ok s
  | _ ->
      fail @@ internal_assertion_failure "trying to untype generated type"
  ...
  let aux
    ...
    let ds' =
      ...
      | None ->
          ds
      | Some d' ->
          ds @ [Location.wrap ~loc:(Location.get_location d) d']
...
let type_and_subst_xyz (env_state_node : environment * Solver.state * 'a)
     ...
     fun ~variable ->
      ...
      @@ let%bind root =
           trace_option
             (simple_error
                (Format.asprintf
                   "can't find alias root of variable %a"
                   Var.pp
                   variable))
           @@
         ...
         let%bind assignment =
           trace_option
             (simple_error
                (Format.asprintf
                   "can't find assignment for root %a"
                   Var.pp
                   root))
...
let type_program (p : I.program) : (O.program * Solver.state) result =
  ...
  type_and_subst_xyz
    ...
    Typesystem.Misc.Substitution.Pattern.s_program
    type_program_returns_state
...
let type_expression_subst (env : environment) (state : Solver.state)
    ?(tv_opt : O.type_expression option) (e : I.expression) :
    (O.expression * Solver.state) result =
  let () = ignore tv_opt in
  (* For compatibility with the old typer's API, this argument can be removed once the new typer is used. *)
  type_and_subst_xyz
  ...
  let aux (env, state) (statement : I.declaration Location.wrap) =
    ...
    let declaration'' =
      ...
      | None ->
          None
      | Some x ->
          Some (Location.wrap ~loc:Location.(statement.location) x)
    in
...
let untype_literal (l : O.literal) : I.literal result =
  ...
  | Literal_key s ->
      ok (Literal_key s)
  | Literal_key_hash s ->
      ok (Literal_key_hash s)
  | Literal_chain_id s ->
      ok (Literal_chain_id s)
  | Literal_signature s ->
      ok (Literal_signature s)
  | Literal_bytes b ->
      ok (Literal_bytes b)
  | Literal_address s ->
      ok (Literal_address s)
  | Literal_operation s ->
      ok (Literal_operation s)

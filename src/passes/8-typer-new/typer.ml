...
module Errors = struct
  ...
  let unbound_variable
    ...
    let data =
      [ ("variable", name);
        ( "environment",
          fun () -> Format.asprintf "%a" Environment.PP.full_environment e );
  ...
  let program_error (p : I.program) () =
    ...
    let data = [("program", fun () -> Format.asprintf "%a" I.PP.program p)] in
    error ~data title message ()

  let constant_declaration_error
      (name : I.expression_variable)
      (ae : I.expr)
      (expected : O.type_expression option)
      () =
    let title = thunk "typing constant declaration" in
    let message () = "" in
    let data =
      [ ( "constant",
          fun () -> Format.asprintf "%a" I.PP.expression_variable name );
        (* Todo : remove Stage_common*)
        ...
        ( "expected",
          fun () ->
            ...
            | Some expected ->
                Format.asprintf "%a" O.PP.type_expression expected );
  ...
  let match_error :
      ?msg:string ->
      expected:I.matching_expr ->
      actual:O.type_expression ->
      Location.t ->
      unit ->
      _ =
   fun ?(msg = "") ~expected ~actual loc () ->
     ...
     let data =
       [ ( "expected",
           fun () -> Format.asprintf "%a" I.PP.matching_type expected );
  ...
  let type_error
    ...
    let data =
      [ ( "expected",
          fun () -> Format.asprintf "%a" O.PP.type_expression expected );
        ("actual", fun () -> Format.asprintf "%a" O.PP.type_expression actual);
        ( "expression",
          fun () -> Format.asprintf "%a" I.PP.expression expression );
        ("location", fun () -> Format.asprintf "%a" Location.pp loc) ]
    in
    error ~data title message ()
end
...
*)
let rec type_declaration env state :
    I.declaration -> (environment * Solver.state * O.declaration option) result
    = function
  ...
  | Declaration_constant (name, tv_opt, inline, expression) ->
    ...
    *)
      ...
      let%bind ae', state' =
        ...
        @@ type_expression env state expression
      in
...
and type_match :
    ...
    I.expression ->
    Location.t ->
    (O.matching_expr * Solver.state) result =
 fun e state t i ae loc ->
   match i with
   | Match_bool {match_true; match_false} ->
       let%bind _ =
         trace_strong (match_error ~expected:i ~actual:t loc) @@ get_t_bool t
       in
   ...
   | Match_option {match_none; match_some} ->
       let%bind t_opt =
         trace_strong (match_error ~expected:i ~actual:t loc) @@ get_t_option t
       in
   ...
   | Match_list {match_nil; match_cons} ->
       let%bind t_elt =
         trace_strong (match_error ~expected:i ~actual:t loc) @@ get_t_list t
       in
   ...
   | Match_tuple ((lst, b), _) ->
       let%bind t_tuple =
         trace_strong (match_error ~expected:i ~actual:t loc) @@ get_t_tuple t
       in
       let%bind lst' =
         generic_try (match_tuple_wrong_arity t_tuple lst loc)
         @@ fun () -> List.combine lst t_tuple
       in
       ...
       let%bind variant_opt =
         let aux acc ((constructor_name, _), _) =
           let%bind _, variant =
             ...
             @@ Environment.get_constructor constructor_name e
           in
           let%bind acc =
             match acc with
             ...
             | Some variant' ->
                 trace
                   (type_error ~msg:"in match variant" ~expected:variant
                      ~actual:variant' ~expression:ae loc)
                 @@ Ast_typed.assert_type_expression_eq (variant, variant')
                 >>? fun () -> ok (Some variant)
           in
           ok acc
         in
         trace (simple_info "in match variant") @@ bind_fold_list aux None lst
       in
       let%bind variant =
         trace_option (match_empty_variant i loc) @@ variant_opt
       in
       let%bind () =
         let%bind variant_cases' =
           trace (match_error ~expected:i ~actual:t loc)
           @@ Ast_typed.Combinators.get_t_sum variant
         in
         let variant_cases =
           List.map fst @@ I.CMap.to_kv_list variant_cases'
         in
         let match_cases = List.map (Function.compose fst fst) lst in
         let test_case c = Assert.assert_true (List.mem c match_cases) in
         let%bind () =
           trace_strong (match_missing_case i loc)
           @@ bind_iter_list test_case variant_cases
         in
         let%bind () =
           trace_strong (match_redundant_case i loc)
           @@ Assert.assert_true
                List.(length variant_cases = length match_cases)
         in
         ok ()
       in
       let%bind state'', lst' =
         ...
         in
         bind_fold_map_list aux state lst
       in
...
*)
and evaluate_type (e : environment) (t : I.type_expression) :
    O.type_expression result =
  ...
  | T_variable name ->
      ...
      in
      ok tv
  | T_constant cst -> return (T_constant cst)
  | T_operator opt ->
      let%bind opt =
        match opt with
        | TC_set s                 ->
...
and type_expression :
 ...
 fun e state ?tv_opt ae ->
   let () = ignore tv_opt in
   (* For compatibility with the old typer's API, this argument can be removed once the new typer is used. *)
   let open Solver in
   let module L = Logger.Stateful () in
   let return : _ -> Solver.state -> _ -> _ (* return of type_expression *) =
    fun expr state constraints type_name ->
      ...
      ok @@ (expr', new_state)
   in
   let return_wrapped expr state (constraints, expr_type) =
   ...
   | E_record_accessor {record; path} ->
       ...
       let wrapped =
         Wrap.access_label ~base:base'.type_expression ~label:path
       in
   ...
   | E_constructor {constructor; element} ->
       let%bind c_tv, sum_tv =
         let error =
           ...
           let content () =
             Format.asprintf "%a in:\n%a\n" Stage_common.PP.constructor
               constructor O.Environment.PP.full_environment e
           in
           error title content
         in
         trace_option error @@ Environment.get_constructor constructor e
       in
       let%bind expr', state' = type_expression e state element in
       let%bind _assert =
         O.assert_type_expression_eq (expr'.type_expression, c_tv)
       in
       let wrapped = Wrap.constructor expr'.type_expression c_tv sum_tv in
       return_wrapped
         (E_constructor {constructor; element= expr'})
         state' wrapped
   (* Record *)
   | E_record m ->
       ...
       in
       let%bind m', state' =
         Stage_common.Helpers.bind_fold_lmap aux (ok (I.LMap.empty, state)) m
       in
   ...
   | E_record_update {record; path; update} ->
       ...
       let%bind wrapped, tv =
         ...
         | T_record record -> (
             ...
             | None    ->
                 failwith
                 @@ Format.asprintf "field %a is not part of record"
                      Stage_common.PP.label path )
         | _               -> failwith
                                "Update an expression which is not a record"
       in
       let%bind () =
         O.assert_type_expression_eq (tv, get_type_expression update)
       in
       return_wrapped
         (E_record_update {record; path; update})
         state (Wrap.record wrapped)
   (* Data-structure *)
   ...
   | E_application {lamb; args} ->
       ...
       let wrapped =
         Wrap.application f'.type_expression args.type_expression
       in
   ...
   | E_let_in {let_binder; rhs; let_result; inline} ->
       let%bind rhs_tv_opt =
         bind_map_option (evaluate_type e) (snd let_binder)
       in
       ...
       let wrapped =
         Wrap.let_in rhs.type_expression rhs_tv_opt let_result.type_expression
       in
       return_wrapped
         (E_let_in {let_binder; rhs; let_result; inline})
         state'' wrapped
   | E_ascription {anno_expr; type_annotation} ->
       ...
       let wrapped =
         Wrap.annotation expr'.type_expression tv
       ...
       in
       return_wrapped expr'.expression_content state' wrapped
   | E_matching {matchee; cases} ->
       let%bind ex', state' = type_expression e state matchee in
       let%bind m', state'' =
         type_match e state' ex'.type_expression cases ae ae.location
       in
       let tvs =
         let aux (cur : (O.expression, O.type_expression) O.matching_content) =
           match cur with
           | Match_bool {match_true; match_false} -> [match_true; match_false]
           | Match_list {match_nil; match_cons= _, _, match_cons, _} ->
               [match_nil; match_cons]
           | Match_option {match_none; match_some= _, match_some, _} ->
               [match_none; match_some]
           | Match_tuple ((_, match_tuple), _) -> [match_tuple]
           | Match_variant (lst, _) -> List.map snd lst
         in
         List.map get_type_expression @@ aux m'
       in
       let%bind () =
         match tvs with
   ...
   | E_lambda lambda ->
       ...
       return_wrapped (E_lambda lambda)
         (* TODO: is the type of the entire lambda enough to access the input_type=fresh; ? *)
         state' wrapped
   ...
   | E_constant {cons_name= name; arguments= lst} ->
       ...
       let lst_annot =
         List.map (fun (x : O.expression) -> x.type_expression) lst
       in
...
(*
      ...
      return (E_constant (name' , lst')) tv
    *)
and type_lambda e state {binder; input_type; output_type; result} =
  ...
  let fresh : O.type_expression =
    t_variable (Solver.Wrap.fresh_binder ()) ()
  in
  let e' = Environment.add_ez_binder binder fresh e in
  let%bind result, state' = type_expression e' state result in
  let () =
    Printf.printf
      "this does not make use of the typed body, this code sounds buggy."
  in
...
let untype_type_value (t : O.type_expression) : I.type_expression result =
  ...
  | Some s -> ok s
  | _      -> fail
              @@ internal_assertion_failure "trying to untype generated type"
...
let type_program_returns_state
    ((env, state, p) : environment * Solver.state * I.program) :
    (environment * Solver.state * O.program) result =
  let aux
      ( (e : environment),
        (s : Solver.state),
        (ds : O.declaration Location.wrap list) )
      (d : I.declaration Location.wrap) =
    let%bind e', s', d'_opt = type_declaration e s (Location.unwrap d) in
    let ds' =
      match d'_opt with
      | None    -> ds
      | Some d' -> ds @ [Location.wrap ~loc:(Location.get_location d) d']
      (* take O(n) insted of O(1) *)
    in
    ok (e', s', ds')
  in
  let%bind env', state', declarations =
    trace (fun () -> program_error p ())
    @@ bind_fold_list aux (env, state, []) p
  in
  let () = ignore (env', state') in
  ok (env', state', declarations)

let type_and_subst_xyz
    (env_state_node : environment * Solver.state * 'a)
    (apply_substs : 'b Typesystem.Misc.Substitution.Pattern.w)
    (type_xyz_returns_state :
      environment * Solver.state * 'a ->
      (environment * Solver.state * 'b) Trace.result) :
    ('b * Solver.state) result =
  ...
  let subst_all =
    ...
    let substs : variable:I.type_variable -> _ =
     fun ~variable ->
       to_option
       @@ let%bind root =
            trace_option
              (simple_error
                 (Format.asprintf "can't find alias root of variable %a" Var.pp
                    variable))
            @@
          ...
          let%bind assignment =
            trace_option
              (simple_error
                 (Format.asprintf "can't find assignment for root %a" Var.pp
                    root))
            @@ Solver.TypeVariableMap.find_opt root assignments
          in
          let Solver.{tv; c_tag; tv_list} = assignment in
          let () =
            ignore tv
            (* I think there is an issue where the tv is stored twice (as a key and in the element itself) *)
          in
          let%bind (expr : O.type_content) =
            Typesystem.Core.type_expression'_of_simple_c_constant
              ( c_tag,
                List.map
                  (fun s -> O.{type_content= T_variable s; type_meta= None})
                  tv_list )
          in
          ok @@ expr
    in
  ...
  let () = ignore env in
  (* TODO: shouldn't we use the `env` somewhere? *)
  ok (program, state)
...
let type_program (p : I.program) : (O.program * Solver.state) result =
  ...
  type_and_subst_xyz
    (empty_env, empty_state, p)
    Typesystem.Misc.Substitution.Pattern.s_program type_program_returns_state

let type_expression_returns_state :
    environment * Solver.state * I.expression ->
    (environment * Solver.state * O.expression) Trace.result =
 fun (env, state, e) ->
   let%bind e, state = type_expression env state e in
   ok (env, state, e)

let type_expression_subst
    (env : environment)
    (state : Solver.state)
    ?(tv_opt : O.type_expression option)
    (e : I.expression) : (O.expression * Solver.state) result =
  let () = ignore tv_opt in
  (* For compatibility with the old typer's API, this argument can be removed once the new typer is used. *)
  type_and_subst_xyz (env, state, e)
    Typesystem.Misc.Substitution.Pattern.s_expression
    type_expression_returns_state
 ...
 *)
let type_program' : I.program -> O.program result =
 fun p ->
   ...
   let aux (env, state) (statement : I.declaration Location.wrap) =
     let statement' = statement.wrap_content in
     (* TODO *)
     let%bind env', state', declaration' =
       type_declaration env state statement'
     in
     let declaration'' =
       match declaration' with
       | None   -> None
       | Some x -> Some (Location.wrap ~loc:Location.(statement.location) x)
     in
     ok ((env', state'), declaration'')
   in
   let%bind (env', state'), p' =
     bind_fold_map_list aux (initial_env, initial_state) p
   in
   let p' =
     List.fold_left
       (fun l e -> match e with None -> l | Some x -> x :: l)
       [] p'
   in
...
*)
let rec untype_type_expression (t : O.type_expression) :
    I.type_expression result =
  (* TODO: or should we use t.core if present? *)
  let%bind t =
    match t.type_content with
    | O.T_sum x ->
        let%bind x' =
          Stage_common.Helpers.bind_map_cmap untype_type_expression x
        in
        ok @@ I.T_sum x'
    | O.T_record x ->
        let%bind x' =
          Stage_common.Helpers.bind_map_lmap untype_type_expression x
        in
        ok @@ I.T_record x'
    | O.T_constant tag -> ok @@ I.T_constant tag
    | O.T_variable name ->
        ok @@ I.T_variable name (* TODO: is this the right conversion? *)
...
let rec untype_expression (e : O.expression) : I.expression result =
  ...
  | E_record r ->
      ...
      let%bind r' = bind_smap @@ Map.String.map untype_expression r in
      return (e_record r')
  | E_record_accessor {record; path} ->
...
*)
and untype_matching :
    (O.expression -> I.expression result) ->
    O.matching_expr ->
    I.matching_expr result =
 fun f m ->
   let open I in
   match m with
   | Match_bool {match_true; match_false} ->

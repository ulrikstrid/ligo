...
let rec check_recursive_call :
    expression_variable -> bool -> expression -> unit result =
 fun n final_path e ->
  ...
  | E_variable v ->
      let%bind _ =
        ...
        @@ Assert.assert_true (final_path || n <> v)
      in
      ok ()
...
and check_recursive_call_in_matching n final_path c =
  match c with

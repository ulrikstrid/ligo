...
let rec fold_expression : 'a folder -> 'a -> expression -> 'a result =
 fun f init e ->
...
and fold_cases : 'a folder -> 'a -> matching_expr -> 'a result =
 fun f init m ->
   match m with
...
let rec map_expression : mapper -> expression -> expression result =
 fun f e ->
...
and map_cases : mapper -> matching_expr -> matching_expr result =
 fun f m ->
   match m with
   | Match_bool {match_true; match_false} ->
...
and map_program : mapper -> program -> program result =
 fun m p ->
...
let rec fold_map_expression :
    'a fold_mapper -> 'a -> expression -> ('a * expression) result =
 fun f a e ->
...
and fold_map_cases :
    'a fold_mapper -> 'a -> matching_expr -> ('a * matching_expr) result =
 fun f init m ->
   match m with
   | Match_bool {match_true; match_false} ->
...
and fold_map_program : 'a fold_mapper -> 'a -> program -> ('a * program) result
    =
 fun m init p ->
   let aux (acc, acc_prg) (x : declaration Location.wrap) =
...
module Errors = struct
  let bad_contract_io entrypoint e () =
    ...
    let data =
      [ ("location", fun () -> Format.asprintf "%a" Location.pp e.location);
        ...
        ( "entrypoint_type",
          fun () ->
            Format.asprintf "%a" Ast_typed.PP.type_expression e.type_expression
        ) ]
    in
  ...
  let expected_list_operation entrypoint got e () =
    ...
    let message () =
      Format.asprintf "expected %a, got %a" Ast_typed.PP.type_expression
        { got with
          type_content=
            T_operator
              (TC_list {got with type_content= T_constant TC_operation}) }
  ...
  let expected_same entrypoint t1 t2 e () =
    ...
    let message () =
      Format.asprintf
        "expected {%a} and {%a} to be the same in the entrypoint type"
        Ast_typed.PP.type_expression t1 Ast_typed.PP.type_expression t2
    in
    let data =
      [ ("location", fun () -> Format.asprintf "%a" Location.pp e.location);
        ("entrypoint", fun () -> entrypoint);
...
type contract_type =
  {parameter: Ast_typed.type_expression; storage: Ast_typed.type_expression}

let fetch_contract_type : string -> program -> contract_type result =
 fun main_fname program ->
   let main_decl =
     List.rev
     @@ List.filter
          (fun declt ->
   ...
   | hd :: _ -> (
       ...
       | T_arrow {type1; type2} -> (
         ...
         | T_record tin, T_record tout
           when is_tuple_lmap tin && is_tuple_lmap tout ->
             ...
             let%bind () =
               trace_strong
                 (Errors.expected_list_operation main_fname listop e)
               @@ Ast_typed.assert_t_list_operation listop
             in
             let%bind () =
               trace_strong
                 (Errors.expected_same main_fname storage storage' e)
         ...
         | _                               -> fail
                                              @@ Errors.bad_contract_io
                                                   main_fname e )
       | _ -> fail @@ Errors.bad_contract_io main_fname e )
   | []         -> simple_fail
                     ("Entrypoint '" ^ main_fname ^ "' does not exist")

 ...
 fun main_fname program ->
  ...
  | hd :: _ -> (
      ...
      | T_arrow {type1; type2} -> (
        ...
        | (T_record tin, T_record tout)
          when is_tuple_lmap tin && is_tuple_lmap tout ->
            let%bind (parameter, storage) =
              Stage_common.Helpers.get_pair tin
            in
            let%bind (listop, storage') = Stage_common.Helpers.get_pair tout in
            let%bind () =
              trace_strong (Errors.expected_list_operation main_fname listop e)
              @@ Ast_typed.assert_t_list_operation listop
            in
            let%bind () =
              trace_strong (Errors.expected_same main_fname storage storage' e)
        ...
        | _ ->
            fail @@ Errors.bad_contract_io main_fname e )
      | _ ->
          fail @@ Errors.bad_contract_io main_fname e )
  | [] ->

...
type contract_pass_data =
  {contract_type: Helpers.contract_type; main_name: string}

module Errors = struct
  let bad_self_type expected got loc () =
    let title = thunk "bad self type" in
    let message () =
      Format.asprintf "expected %a but got %a" Ast_typed.PP.type_expression
        expected Ast_typed.PP.type_expression got
    in
    let data =
  ...
  let bad_format_entrypoint_ann ep loc () =
    ...
    let message () =
      Format.asprintf
        "entrypoint \"%s\" is badly formatted. We expect \"%%bar\" for \
         entrypoint Bar and \"%%default\" when no entrypoint used"
        ep
    in
    let data =
      [("location", fun () -> Format.asprintf "%a" Location.pp loc)]
...
let check_entrypoint_annotation_format ep exp =
  ...
  | [""; ep'] ->
      ...
      if String.equal cap ep' then
        fail @@ Errors.bad_format_entrypoint_ann ep exp.location
      else
        ok cap
...
let self_typing :
 ...
 fun dat e ->
   let bad_self_err () =
     Errors.bad_self_type e.type_expression
       { e.type_expression with
         type_content= T_operator (TC_contract dat.contract_type.parameter) }
       e.location
   in
   match (e.expression_content, e.type_expression) with
   | ( E_constant {cons_name= C_SELF; arguments= [entrypoint_exp]},
       {type_content= T_operator (TC_contract t); type_meta= _} ) ->
       let%bind entrypoint =
         match entrypoint_exp.expression_content with
         | E_literal (Literal_string ep) ->
             check_entrypoint_annotation_format ep entrypoint_exp
         | _                             ->
             fail
             @@ Errors.entrypoint_annotation_not_literal
                  entrypoint_exp.location
       in
       let%bind entrypoint_t =
         match dat.contract_type.parameter.type_content with
         | T_sum cmap ->
             trace_option (Errors.unmatched_entrypoint entrypoint_exp.location)
       ...
       let%bind () =
         ...
         @@ Ast_typed.assert_type_expression_eq (entrypoint_t, t)
       in
       ok (true, dat, e)
   | _ -> ok (true, dat, e)

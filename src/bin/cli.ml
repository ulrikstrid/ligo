...
let print_ast_typed =
  let f source_file syntax typer_switch protocol_version display_format =
    return_result ~display_format (Ast_typed.Formatter.module_format_fully_typed) @@
      ...
      in
      let%bind (typed,_),_ = Build.type_contract ~options syntax Env source_file in
      ok @@ typed
  in
  ...
  let f source_file syntax typer_switch protocol_version display_format =
    return_result ~display_format (Ast_typed.Formatter.module_format_fully_typed) @@
      let%bind options =
        ...
        ok @@ Compiler_options.make ~typer_switch ~init_env ~protocol_version ()
      in
      let%bind typed,_,_ = Build.combined_contract ~options syntax Env source_file in
      ok @@ typed
  in
  let term = Term.(const f $ source_file 0  $ syntax $ typer_switch $ protocol_version $ display_format) in
  ...
  let f source_file syntax typer_switch protocol_version display_format optimize =
    return_result ~display_format (Mini_c.Formatter.program_format) @@
      let%bind options =
        ...
        ok @@ Compiler_options.make ~typer_switch ~init_env ~protocol_version ()
      in
      let%bind mini_c,_,_ = Build.build_mini_c ~options syntax Env source_file in
      match optimize with
...
let compile_parameter =
  let f source_file entry_point expression syntax typer_switch protocol_version amount balance sender source now display_format michelson_format output_file warn =
    return_result ~warn ~output_file ~display_format (Formatter.Michelson_formatter.michelson_format michelson_format) @@
      ...
      let%bind typer_switch = Helpers.typer_switch_to_variant typer_switch in
      let options = Compiler_options.make ~typer_switch ~init_env () in
      let%bind typed_prg,env,_   = Build.combined_contract ~options syntax (Contract entry_point) source_file in
...
let interpret =
  let f expression init_file syntax typer_switch protocol_version amount balance sender source now display_format =
    return_result ~display_format (Decompile.Formatter.expression_format) @@
      ...
      let%bind (decl_list,mods,state,env) = match init_file with
        | Some init_file ->
           let%bind mini_c_prg,mods,_,env,typer_state = Build.build_contract_use ~options syntax init_file in
          ok (mini_c_prg,mods,typer_state,env)
        | None -> ok ([],Ast_core.SMap.empty,Typer.Solver.initial_state,init_env) in
...
let dry_run =
  let f source_file entry_point storage input amount balance sender source now syntax typer_switch protocol_version display_format warn =
    return_result ~warn ~display_format (Decompile.Formatter.expression_format) @@
      ...
      let%bind typer_switch = Helpers.typer_switch_to_variant typer_switch in
      let options = Compiler_options.make ~typer_switch ~init_env () in
      let%bind mini_c_prg,_,typed_prg,env,state = Build.build_contract_use  ~options syntax source_file in
      let%bind michelson_prg   = Compile.Of_mini_c.aggregate_and_compile_contract ~options mini_c_prg entry_point in
      let%bind _contract =
        (* fails if the given entry point is not a valid contract *)
        Compile.Of_michelson.build_contract michelson_prg in
...
let run_function =
  let f source_file entry_point parameter amount balance sender source now syntax typer_switch protocol_version display_format =
    return_result ~display_format (Decompile.Formatter.expression_format) @@
      ...
      let%bind typer_switch = Helpers.typer_switch_to_variant typer_switch in
      let options = Compiler_options.make ~typer_switch ~init_env () in
      let%bind mini_c_prg,mods,typed_prg,env,state = Build.build_contract_use ~options syntax source_file in
...
let evaluate_value =
  let f source_file entry_point amount balance sender source now syntax typer_switch protocol_version display_format =
    return_result ~display_format Decompile.Formatter.expression_format @@
      ...
      let%bind typer_switch = Helpers.typer_switch_to_variant typer_switch in
      let options = Compiler_options.make ~typer_switch ~init_env () in
      let%bind mini_c,_,typed_prg,_,_ = Build.build_contract_use ~options syntax source_file in
  ...
  let f expression syntax typer_switch protocol_version init_file display_format michelson_format warn =
    return_result ~warn ~display_format (Formatter.Michelson_formatter.michelson_format michelson_format) @@
      ...
      let%bind (decl_list,env) = match init_file with
        | Some init_file ->
          let%bind mini_c_prg,env,_  = Build.build_mini_c ~options syntax Env init_file  in
          ok (mini_c_prg,env)
        | None -> ok ([],init_env) in

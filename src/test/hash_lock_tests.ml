...
let storage hashed used commits =
  e_record_ez
    [("hashed", hashed); ("unused", e_bool used); ("commits", commits)]
...
let commit () =
  ...
  let salted_hash =
    e_bytes_raw
      (sha_256_hash (Bytes.concat Bytes.empty [test_hash_raw; packed_sender]))
  in
  let pre_commits =
    e_typed_big_map [] t_address
      (t_record_ez [("date", t_timestamp); ("salted_hash", t_bytes)])
  in
  let init_storage = storage test_hash true pre_commits in
  let commit =
    e_record_ez
      [ ("date", e_timestamp (Int64.to_int (to_sec lock_time)));
  ...
  let options =
    Proto_alpha_utils.Memory_proto_alpha.make_options ~predecessor_timestamp
...
let reveal_breaks_commit () =
  ...
  let salted_hash =
    e_bytes_raw
      (sha_256_hash
         (Bytes.concat Bytes.empty [Bytes.of_string "hello"; packed_sender]))
  in
  let commit =
    e_record_ez
      [ ("date", e_timestamp (Int64.to_int (to_sec predecessor_timestamp)));
        ("salted_hash", salted_hash) ]
  in
  let commits = e_big_map [(e_address first_committer, commit)] in
  let init_storage = storage test_hash true commits in
...
let reveal_wrong_commit () =
  ...
  let reveal =
    e_record_ez
      [("hashable", e_bytes_string "hello"); ("message", empty_message)]
  in
  let%bind predecessor_timestamp = mk_time "2000-01-01T00:10:10Z" in
  let test_hash_raw = sha_256_hash (Bytes.of_string "hello world") in
  let test_hash = e_bytes_raw test_hash_raw in

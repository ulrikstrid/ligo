...
let pledge () =
  ...
  let options =
    Proto_alpha_utils.Memory_proto_alpha.make_options ~sender:oracle_contract
      ~amount:Memory_proto_alpha.Protocol.Alpha_context.Tez.one ()
  in
  expect_eq ~options program "donate" (e_pair parameter storage)
...
let distribute () =
  ...
  let options =
    Proto_alpha_utils.Memory_proto_alpha.make_options ~sender:oracle_contract
      ()
  in
  expect_eq ~options program "distribute" (e_pair parameter storage)
    (e_pair (e_list []) storage)
...
let distribute_unauthorized () =
  ...
  let parameter = empty_message in
  let options =
    Proto_alpha_utils.Memory_proto_alpha.make_options ~sender:stranger_contract
      ()
  in
  expect_string_failwith ~options program "distribute"
    (e_pair parameter storage) "You're not the oracle for this distribution."

let main =
  test_suite "Pledge & Distribute"
    [ test "donate" pledge;

...
type storage = identities: (id, id_details) big_map
...
annoying than an IP address and a lot more memorable than the raw digits. In this
case ID's could be a tuple of 5 integers from 0 to 255. They could also be a single
integer which is then translated into the corresponding series of 5 words.
...
let buy (parameter: (bytes * address option) * storage) =
  ...
  let controller: address =
    ...
    | None -> sender
  in
  let new_id: id = gen_name profile in
  let untaken: bool =
    match Big_map.find_opt new_name parameter.1 with
  ...
  let updated_identities: (id, id_details) big_map =
    Big_map.update new_id new_id_details parameter.1
  in
  in ([]: instruction, updated_identities)

let update_owner (parameter: (id * address) * storage) =
  let (id: id), (new_owner: address) = paramater.0 in
  let identities = parameter.1 in
...
let update_details (parameter: (id * bytes * address option) * storage) =
  let current_id_details = Big_map.find_opt parameter.0.0 parameter.1 in
  let is_allowed: bool =
    if
  ...
  in
  let identities: (id, id_details) big_map = parameter.1 in
...
let whois_id (parameter: id * storage) =
  ...
  in () (* Not entitrely sure how to return the result here *)

...
type test = Test_suite of (string * test list) | Test of test_case
...
let pack_payload (program : Ast_typed.program) (payload : expression) :
    bytes result =
  let%bind code =
    ...
    Compile.Of_mini_c.compile_expression mini_c
  in
  let (Ex_ty payload_ty) = code.expr_ty in
  let%bind (payload : Tezos_utils.Michelson.michelson) =
    Ligo.Run.Of_michelson.evaluate_expression code.expr code.expr_ty
  in
  Ligo.Run.Of_michelson.pack_payload payload payload_ty

let sign_message (program : Ast_typed.program) (payload : expression) sk :
    string result =
...
let expect_eq ?options program entry_point input expected =
  ...
  let expecter result =
    let expect_error =
      ...
      error title content
    in
    trace expect_error @@ Ast_core.Misc.assert_value_eq (expected, result)
  in
...
let expect_eq_evaluate program entry_point expected =
  let%bind expected = expression_to_core expected in
  let expecter result = Ast_core.Misc.assert_value_eq (expected, result) in
...
let expect_eq_n_int a b c = expect_eq_n a b e_int (fun n -> e_int (c n))

let expect_eq_b_bool a b c = expect_eq_b a b (fun bool -> e_bool (c bool))

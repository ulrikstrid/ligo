...
let get_program =
  ...
  fun () -> match !s with
    ...
    | None -> (
        ...
        let%bind (program , state) = mtype_file "./contracts/access_control.religo" in
...
let compile_main () =
  ...
  let%bind simplified      = Ligo.Compile.Of_source.compile "./contracts/access_control.religo" (Syntax_name "cameligo") in
...
let main = test_suite "Access Control (ReasonLIGO)" [

...
let compile_main f s () =
  ...
  let%bind (_contract : Tezos_utils.Michelson.michelson) =
    ...
    Ligo.Compile.Of_michelson.build_contract michelson_prg
  in
  ok ()
...
let params counter msg keys is_validl f s =
  ...
  let aux acc (key, is_valid) =
    ...
    ok @@ (e_pair (e_key_hash pkh) (e_signature signature) :: acc)
  in
  let%bind signed_msgs =
    Trace.bind_fold_list aux [] (List.rev @@ List.combine keys is_validl)
  in
  ok
  @@ e_constructor "CheckMessage"
       (e_record_ez
          [ ("counter", e_nat counter);
            ("message", msg);
            ( "signatures",
...
let not_enough_1_of_2 f s () =
  ...
  let%bind () =
    expect_string_failwith program "main"
...
let valid_1_of_1 f s () =
  ...
  let keys = gen_keys () in
  let%bind () =
    expect_eq_n_trace_aux [0; 1; 2] program "main"
      (fun n ->
        let%bind params = params n empty_message [keys] [true] f s in
        ok @@ e_pair params (init_storage 1 n [keys]))
      (fun n -> ok @@ e_pair empty_op_list (init_storage 1 (n + 1) [keys]))
  in
  ok ()
...
let valid_2_of_3 f s () =
  ...
  let%bind () =
    expect_eq_n_trace_aux [0; 1; 2] program "main"
      ...
      (fun n -> ok @@ e_pair empty_op_list (init_storage 2 (n + 1) st_keys))
  in
  ok ()
...
let invalid_3_of_3 f s () =
  ...
  let%bind test_params =
    params 0 empty_message param_keys [false; true; true] f s
  in
  let exp_failwith = "Invalid signature" in
  let%bind () =
    expect_string_failwith program "main"
...
let main =
  test_suite "Multisig"
    [ test "compile" (compile_main file "pascaligo");
      ...
      test "unmatching_counter (religo)"
        (unmatching_counter refile "reasonligo");
      test "valid_1_of_1 (religo)" (valid_1_of_1 refile "reasonligo");
      test "invalid_1_of_1 (religo)" (invalid_1_of_1 refile "reasonligo");
      test "not_enough_signature (religo)"
        (not_enough_1_of_2 refile "reasonligo");

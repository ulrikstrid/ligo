...
let retype_file f =
  let%bind simplified  = Ligo.Compile.Of_source.compile ~loc_form:(Location.file) f (Syntax_name "reasonligo") in
  let%bind typed,state = Ligo.Compile.Of_simplified.compile simplified in
  let () = Typer.Solver.discard_state state in
  let () = Typer.Solver.discard_state state in
  ok typed
let mtype_file f =
  let%bind simplified  = Ligo.Compile.Of_source.compile ~loc_form:(Location.file) f (Syntax_name "cameligo") in
  let%bind typed,state = Ligo.Compile.Of_simplified.compile simplified in
  let () = Typer.Solver.discard_state state in
  ok typed
let type_file f =
  let%bind simplified  = Ligo.Compile.Of_source.compile ~loc_form:(Location.file) f (Syntax_name "pascaligo") in
  let%bind typed,state = Ligo.Compile.Of_simplified.compile simplified in
  let () = Typer.Solver.discard_state state in
  ok typed

...
let anon_function () : unit result =
  ...
  let%bind () = expect_eq_evaluate program "x" (e_int 42) in
  ok ()
...
let application () : unit result =
  ...
  let%bind () =
    ...
    expect_eq_evaluate program "x" expected
  in
  let%bind () =
    let expected = e_int 42 in
    expect_eq_evaluate program "y" expected
  in
  let%bind () =
    let expected = e_int 42 in
    expect_eq_evaluate program "z" expected
  in
  ok ()
...
let variant () : unit result =
  ...
  let%bind () =
    ...
    expect_eq_evaluate program "foo" expected
  in
  let%bind () =
    let expected = e_constructor "Bar" (e_bool true) in
    expect_eq_evaluate program "bar" expected
  in
  let%bind () =
    let expected = e_constructor "Kee" (e_nat 23) in
    expect_eq_evaluate program "kee" expected
  in
  ok ()
...
let variant_matching () : unit result =
  ...
  let%bind () =
    ...
    expect_eq program "fb" (make_input 0) (make_expected 0)
    >>? fun () ->
    expect_eq_n program "fb" make_input make_expected
    >>? fun () ->
    expect_eq program "fb" (e_constructor "Kee" (e_nat 50)) (e_int 23)
    >>? fun () ->
    expect_eq program "fb" (e_constructor "Bar" (e_bool true)) (e_int 42)
    >>? fun () -> ok ()
  in
  ok ()
...
let shared_function () : unit result =
  ...
  in
  let%bind () = expect_eq program "double_inc" (e_int 0) (e_int 2) in
  let%bind () =
...
let arithmetic () : unit result =
  ...
  let%bind () =
    expect_eq_n_pos program "mod_op" e_int (fun n -> e_nat (n mod 42))
  in
  let%bind () =
    expect_eq_n_pos program "div_op" e_int (fun n -> e_int (n / 2))
  in
  ok ()
...
let string_arithmetic () : unit result =
  ...
  let%bind () =
    expect_eq program "concat_op" (e_string "foo") (e_string "foototo")
  in
  let%bind () =
    expect_eq program "concat_op" (e_string "") (e_string "toto")
  in
  let%bind () =
    expect_eq program "slice_op" (e_string "tata") (e_string "at")
  in
  let%bind () =
...
let set_arithmetic () : unit result =
  ...
  in
  let%bind () =
    expect_eq program_1 "fold_op" (e_set [e_int 4; e_int 10]) (e_int 29)
  in
  ok ()
...
let tuple_ez_int names n = e_tuple @@ List.map (fun _ -> e_int n) names
...
let record () : unit result =
  ...
  let%bind () =
    ...
    let make_expected n =
      e_record_ez
        [ ( "inner",
            e_record_ez [("a", e_int n); ("b", e_int 2048); ("c", e_int n)] )
        ]
    in
...
let tuple () : unit result =
  ...
  let ez n = e_tuple (List.map e_int n) in
  let%bind () =
...
let map_ type_f path : unit result =
  ...
  in
  let%bind () =
    let make_input n = ez List.(map (fun x -> (x, x)) @@ range n) in
...
let rebig_map () : unit result =
  big_map_ retype_file "./contracts/big_map.religo"
...
let list () : unit result =
  ...
  in
  let%bind () =
    let make_input n = ez @@ List.range n in
    let make_expected = e_nat in
    expect_eq_n_strict_pos_small program "size_" make_input make_expected
  in
  let%bind () =
  ...
  in
  let%bind () =
    expect_eq program "fold_op" (e_list [e_int 2; e_int 4; e_int 7]) (e_int 23)
  in
  let%bind () =
...
let condition () : unit result =
  ...
  in
  let%bind _ =
    let make_expected b = e_int (if b then 42 else 1) in
...
let loop () : unit result =
  ...
  let%bind () =
    ...
    expect_eq program "for_collection_map_kv" input expected
  in
  let%bind () =
    let expected = e_string "123" in
    expect_eq program "for_collection_map_k" input expected
  in
  let%bind () =
  ...
  let%bind () =
    ...
    expect_eq program "for_collection_if_and_local_var" input expected
  in
  let%bind () =
    let expected = e_int 1020 in
    expect_eq program "for_collection_rhs_capture" input expected
  in
  let%bind () =
    let expected = e_int 1040 in
    expect_eq program "for_collection_proc_call" input expected
  in
  let%bind () =
    let expected = e_int 20 in
    expect_eq program "for_collection_comp_with_acc" input expected
  in
  let%bind () =
    let expected =
      e_pair
    ...
    in
    expect_eq program "nested_for_collection" input expected
  in
  let%bind () =
    let expected = e_pair (e_int 24) (e_string "123123123") in
    expect_eq program "nested_for_collection_local_var" input expected
  in
  let%bind () =
    let expected = e_pair (e_bool true) (e_int 4) in
    expect_eq program "inner_capture_in_conditional_block" input expected
  in
  let%bind () =
    let ez lst =
    ...
    expect_eq program "for_collection_with_patches" input expected
  in
  ok ()
...
let matching () : unit result =
  ...
  let%bind () =
    let aux n =
      let input =
        match n with Some s -> e_some (e_int s) | None -> e_typed_none t_int
      in
      let expected = e_int (match n with Some s -> s | None -> 23) in
      trace
        (simple_error (Format.asprintf "on input %a" PP_helpers.(option int) n))
      @@ expect_eq program "match_option" input expected
    in
    bind_iter_list aux [Some 0; Some 2; Some 42; Some 163; Some (-1); None]
  in
  let%bind () =
    let aux n =
      let input =
        match n with Some s -> e_some (e_int s) | None -> e_typed_none t_int
      in
      let expected = e_int (match n with Some s -> s | None -> 42) in
      trace
        (simple_error (Format.asprintf "on input %a" PP_helpers.(option int) n))
  ...
  let%bind () =
    ...
    let%bind () =
      expect_eq program "match_expr_list" (aux [14; 2; 3]) (e_int 14)
    in
    let%bind () =
      expect_eq program "match_expr_list" (aux [13; 2; 3]) (e_int 13)
    in
...
let declarations () : unit result =
  ...
  expect_eq program "main" (make_input 0) (make_expected 0)
  >>? fun () -> expect_eq_n program "main" make_input make_expected
...
let failwith_ligo () : unit result =
  ...
  in
  let%bind _ =
    should_work (e_pair (e_constructor "Zero" (e_nat 0)) (e_unit ()))
  in
  let%bind _ =
    should_fail (e_pair (e_constructor "Zero" (e_nat 1)) (e_unit ()))
  in
  let%bind _ =
    should_work (e_pair (e_constructor "Pos" (e_nat 1)) (e_unit ()))
  in
  let%bind _ =
    should_fail (e_pair (e_constructor "Pos" (e_nat 0)) (e_unit ()))
  in
  let should_fail input = expect_fail program "foobar" (e_int input) in
  let should_work input n =
    expect_eq program "foobar" (e_int input) (e_int n)
  in
...
let match_variant () : unit result =
  ...
  let%bind () =
    ...
    expect_eq program "match_bool" input expected
  in
  let%bind () =
  ...
  let%bind () =
    ...
    expect_eq program "match_list" input expected
  in
  let%bind () =
  ...
  let%bind () =
    ...
    expect_eq_n program "match_option" make_input make_expected
  in
  ok ()
...
let mligo_list () : unit result =
  ...
  let%bind () =
    expect_eq program "size_" (e_list [e_int 0; e_int 1; e_int 2]) (e_nat 3)
  in
  ...
  let%bind () =
    expect_eq_evaluate program "y" (e_list @@ List.map e_int [3; 4; 5])
  in
  let%bind () =
    expect_eq_evaluate program "z" (e_list @@ List.map e_int [2; 3; 4; 5])
  in
  let%bind () =
    expect_eq program "map_op" (aux [2; 3; 4; 5]) (aux [3; 4; 5; 6])
  in
...
let tez_ligo () : unit result =
  ...
  let%bind _ =
    expect_eq_evaluate program "not_enough_tez" (e_mutez 4611686018427387903)
  in
...
let balance_test_options () =
  let%bind balance =
    trace_option (simple_error "could not convert balance")
    @@ Memory_proto_alpha.Protocol.Alpha_context.Tez.of_string "4000000"
  in
...
let addr_test program =
  ...
  let addr =
    ...
    @@ (List.nth dummy_environment.identities 0).implicit_contract
  in
  let open Tezos_crypto in
  let key_hash =
    Signature.Public_key_hash.to_b58check
    @@ (List.nth dummy_environment.identities 0).public_key_hash
  in
...
let deep_access_ligo () : unit result =
  ...
  let%bind () =
    ...
    expect_eq program "asymetric_tuple_access" make_input make_expected
  in
  let%bind () =
    let make_input =
      e_record_ez
    ...
    expect_eq program "nested_record" make_input make_expected
  in
  ok ()
...
let curry () : unit result =
  ...
  let%bind () = expect_eq program "main" (e_int 2) (e_int 12) in
  let%bind () = expect_eq program "partial_apply" (e_int 2) (e_int 12) in
  ok ()
...
let set_delegate () : unit result =
  ...
  let%bind () =
    expect_eq program "main" (e_key_hash pkh_str) (e_typed_list [] t_operation)
  in
  ok ()
...
let type_tuple_destruct () : unit result =
  ...
  let%bind () =
    expect_eq program "type_tuple_d_2" (e_unit ()) (e_string "helloworld")
  in
  ok ()
...
let tuple_param_destruct () : unit result =
  ...
  let%bind () =
    expect_eq program "sum" (e_tuple [e_int 20; e_int 10]) (e_int 10)
  in
  let%bind () =
    expect_eq program "parentheses" (e_tuple [e_int 20; e_int 10]) (e_int 10)
  in
  ok ()
...
let bytes_unpack () : unit result =
  ...
  in
  let%bind () =
    expect_eq program "id_address" (e_address addr) (e_some (e_address addr))
  in
  ok ()
...
let loop_bugs_ligo () : unit result =
  ...
  let%bind () =
    ...
    expect_eq program "shadowing_in_body" input expected
  in
  let%bind () =
    let expected = e_string "toto" in
    expect_eq program "shadowing_assigned_in_body" input expected
  in
  ok ()

...
module CMap = Map.Make (struct
  ...
  let compare (Constructor a) (Constructor b) = compare a b
end)

module LMap = Map.Make (struct
  type t = label

  let compare (Label a) (Label b) = String.compare a b
end)
...
module Ast_generic_type (PARAMETER : AST_PARAMETER_TYPE) = struct
  ...
  let map_type_operator f = function
    ...
    | TC_big_map (x, y) ->
        TC_big_map (f x, f y)
    | TC_map_or_big_map (x, y) ->
        TC_map_or_big_map (f x, f y)
    | TC_arrow (x, y) ->
        TC_arrow (f x, f y)
  ...
  let type_expression'_of_string = function
    | ("TC_contract", [x]) ->
        ok @@ T_operator (TC_contract x)
    | ("TC_option", [x]) ->
        ok @@ T_operator (TC_option x)
    | ("TC_list", [x]) ->
        ok @@ T_operator (TC_list x)
    | ("TC_set", [x]) ->
        ok @@ T_operator (TC_set x)
    | ("TC_map", [x; y]) ->
        ok @@ T_operator (TC_map (x, y))
    | ("TC_big_map", [x; y]) ->
        ok @@ T_operator (TC_big_map (x, y))
    | ( ( "TC_contract"
        | "TC_option"
        | "TC_list"
        | "TC_set"
    ...
    | ("TC_unit", []) ->
        ok @@ T_constant TC_unit
    | ("TC_string", []) ->
        ok @@ T_constant TC_string
    | ("TC_bytes", []) ->
        ok @@ T_constant TC_bytes
    | ("TC_nat", []) ->
        ok @@ T_constant TC_nat
    | ("TC_int", []) ->
        ok @@ T_constant TC_int
    | ("TC_mutez", []) ->
        ok @@ T_constant TC_mutez
    | ("TC_bool", []) ->
        ok @@ T_constant TC_bool
    | ("TC_operation", []) ->
        ok @@ T_constant TC_operation
    | ("TC_address", []) ->
        ok @@ T_constant TC_address
    | ("TC_key", []) ->
        ok @@ T_constant TC_key
    | ("TC_key_hash", []) ->
        ok @@ T_constant TC_key_hash
    | ("TC_chain_id", []) ->
        ok @@ T_constant TC_chain_id
    | ("TC_signature", []) ->
        ok @@ T_constant TC_signature
    | ("TC_timestamp", []) ->
        ok @@ T_constant TC_timestamp
    | (_, []) ->
  ...
  let string_of_type_expression' = function
    | T_operator o ->
        string_of_type_operator o
    | T_constant c ->
        string_of_type_constant c
...
and ('a, 'tv) matching_content =
  | Match_bool of {match_true : 'a; match_false : 'a}
  | Match_list of {
      match_nil : 'a;

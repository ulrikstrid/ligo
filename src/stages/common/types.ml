...
module Ast_generic_type (PARAMETER : AST_PARAMETER_TYPE) = struct
  ...
  let type_expression'_of_string = function
    ...
    | ( ( "TC_contract" | "TC_option" | "TC_list" | "TC_set" | "TC_map"
        | "TC_big_map" ),
        _ ) ->
    ...
    | _ -> failwith "internal error: unknown type operator"
...
and ('a, 'tv) matching_content =
  ...
  | Match_option of {match_none: 'a; match_some: expression_variable * 'a * 'tv}

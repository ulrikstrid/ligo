...
| ModuleDecl {value={kwd_module; name; eq; lbrace; structure; rbrace}; _} ->
...
| TModA   ma -> print_mod_access print_type_expr state ma
...
and print_some_app_pattern state {value; _} =
  ...
  print_token   state ctor_Some "Some";
...
| EModA ma            -> print_mod_access print_expr state ma
...
and print_mod_in state {value; _} =
  ...
  let {kwd_module; name; eq; lbrace; structure; rbrace} = mod_decl in
  print_token  state kwd_module "module";
  print_var    state name;
  print_token  state eq     "eq";
  print_token  state lbrace "{";
...
and print_conditional state {value; _} =
  ...
  let print state (expr, semi) =
    print_expr      state expr;
    print_token_opt state semi ";" in
  print_token  state ghost "(";
  print_token  state kwd_if "if";
  print_expr   state test;
  print_braced state print ifso;
  print_option state (fun state (kwd_else, ifnot) ->
                        print_token  state kwd_else "else";
                        print_braced state print ifnot) ifnot;
  print_token state ghost ")"
...
and pp_module_alias state decl =
  let path           = Utils.nsepseq_to_list decl.mod_path in
...
and pp_mod_alias state node =
  ...
  let () =
    let state = state#pad 3 1 in
    let path           = Utils.nsepseq_to_list mod_path in
    let len            = List.length path in
    let apply len rank = pp_ident (state#pad len rank) in
    pp_node state "<module>";
    List.iteri (apply len) path in
  let () =
    let state = state#pad 3 2 in
    pp_node state "<body>";
    pp_expr (state#pad 1 0) body

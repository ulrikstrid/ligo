...
let () =
  let some_root : root =
    A {a1 = X (A {a1 = X (B 1); a2 = W ()}); a2 = Z (W ())}
  in
  let op =
    {
      no_op with
      a =
        (fun the_a state continue_fold ->
  ...
  if state != 2 then
    failwith
      (Printf.sprintf
         "Test failed: expected folder to count 2 nodes, but it counted %d \
          nodes"
         state)
  else ()
...
let () =
  ...
  in
  let op =
    {no_op with a_post_state = (fun _the_a _new_a state -> state + 1)}
  in
  let state = 0 in
  let (_, state) = fold_root op some_root state in
  if state != 2 then

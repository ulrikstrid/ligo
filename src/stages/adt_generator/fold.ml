...
type root' = A' of a' | B' of int | C' of string

and a' = {a1': ta1'; a2': ta2'}

and ta1' = X' of root' | Y' of ta2'

and ta2' = Z' of ta2' | W' of unit
...
let rec mk_continue_fold : type state. state fold_config -> state continue_fold
    =
 fun visitor ->
...
and fold_root : type state. state fold_config -> root -> state -> root' * state
    =
 fun visitor x state ->
...
and fold_root_A : type state. state fold_config -> a -> state -> a' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.root_A x state continue_fold

and fold_root_B : type state. state fold_config -> int -> state -> int * state
    =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
...
and fold_root_C :
    type state. state fold_config -> string -> state -> string * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.root_C x state continue_fold

and fold_a : type state. state fold_config -> a -> state -> a' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
...
and fold_a_a1 : type state. state fold_config -> ta1 -> state -> ta1' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.a_a1 x state continue_fold

and fold_a_a2 : type state. state fold_config -> ta2 -> state -> ta2' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.a_a2 x state continue_fold

and fold_ta1 : type state. state fold_config -> ta1 -> state -> ta1' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
...
and fold_ta1_X :
    type state. state fold_config -> root -> state -> root' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.ta1_X x state continue_fold

and fold_ta1_Y : type state. state fold_config -> ta2 -> state -> ta2' * state
    =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.ta1_Y x state continue_fold

and fold_ta2 : type state. state fold_config -> ta2 -> state -> ta2' * state =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
...
and fold_ta2_Z : type state. state fold_config -> ta2 -> state -> ta2' * state
    =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
   visitor.ta2_Z x state continue_fold

and fold_ta2_W : type state. state fold_config -> unit -> state -> unit * state
    =
 fun visitor x state ->
   let continue_fold : state continue_fold = mk_continue_fold visitor in
...
let no_op : 'a fold_config =
  { root=
      (fun v state continue ->
        match v with
        | A v ->
            let v, state = continue.root_A v state in
            (A' v, state)
        | B v ->
            let v, state = continue.root_B v state in
            (B' v, state)
        | C v ->
            let v, state = continue.root_C v state in
            (C' v, state));
    ...
    ta1=
      (fun v state continue ->
        match v with
        | X v ->
            let v, state = continue.ta1_X v state in
            (X' v, state)
        | Y v ->
            let v, state = continue.ta1_Y v state in
            (Y' v, state));
    ...
    ta2=
      (fun v state continue ->
        match v with
        | Z v ->
            let v, state = continue.ta2_Z v state in
            (Z' v, state)
        | W v ->
            let v, state = continue.ta2_W v state in
            (W' v, state));

...
let option_to_context : unit -> t =
  fun () ->
    { contracts = StateMap.empty ;
      step_constants = { source = 0 ; payer = 0 ; self = 0 ; amount = Tez.zero ; balance = Tez.zero ; now = Types.Timestamp.zero ; chain_id = Bytes.empty ; origination_counter = 0 }
    }
...
let rec type_expression' :
 ...
 fun f ppf te ->
  ...
  | T_record m ->
      fprintf ppf "{%a}" (record_sep f (const ";")) m
  | T_tuple t ->
      fprintf ppf "(%a)" (list_sep_d f) t
  | T_arrow a ->
...
and option_map ppf (k, v_opt) =
  match v_opt with
  | None ->
      fprintf ppf "%a" expression_variable k
  | Some v ->
...
and option_mut ppf mut = if mut then fprintf ppf "[@mut]" else fprintf ppf ""

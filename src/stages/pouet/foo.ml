...
module Types = struct
  ...
  let foo_expression : 'a Display.format = {
    pp = foo_expression_ppformat;
    to_json = foo_expression_jsonformat;
  }
end

let f_err : unit -> (Types.expression, [> error]) result =
  fun () -> Error (thunk (`Foo_error1 42))
let f_ok  : unit -> (Types.expression, [> error]) result =
  fun () -> Ok (E_toto 42)
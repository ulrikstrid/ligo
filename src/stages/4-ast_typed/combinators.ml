...
let t_contract t ?s () : type_expression =
  make_t (T_operator (TC_contract t)) s
...
let t_pair a b ?s () : type_expression =
  ez_t_record [(Label "0", a); (Label "1", b)] ?s ()
...
let t_map_or_big_map key value ?s () =
  make_t (T_operator (TC_map_or_big_map (key, value))) s

let t_sum m ?s () : type_expression = make_t (T_sum m) s

let make_t_ez_sum (lst : (constructor' * type_expression) list) :
    type_expression =
...
let assert_t_bytes t =
  let%bind _ = get_t_bytes t in
  ok ()

let assert_t_string t =
...
let e_a_pair a b =
  make_a_e (e_pair a b) (t_pair a.type_expression b.type_expression ())

let e_a_some s = make_a_e (e_some s) (t_option s.type_expression ())

let e_a_lambda l in_ty out_ty =
  make_a_e (e_lambda l) (t_function in_ty out_ty ())

let e_a_none t = make_a_e (e_none ()) (t_option t ())

let e_a_record r =
  make_a_e (e_record r) (t_record (LMap.map get_type_expression r) ())
...
let e_a_let_in binder expr body attributes =
  make_a_e (e_let_in binder expr body attributes) (get_type_expression body)

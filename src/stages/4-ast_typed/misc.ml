  ...
  let different_operator_number_of_arguments opa opb lena lenb () =
    ...
    let message () =
      Format.asprintf
        ...
        (type_operator_name opa) lena lenb
    in
    let data =
      [ ( "a",
          fun () ->
...
module Free_variables = struct
  ...
  and matching :
   ...
   fun f b m ->
     ...
     | Match_tuple ((lst, a), _) -> f (union (of_list lst) b) a
     | Match_variant (lst, _) ->
...
let rec assert_type_expression_eq ((a, b) : type_expression * type_expression)
  ...
  | T_operator opa, T_operator opb ->
      let%bind lsta, lstb =
        ...
        | ( (TC_map (ka, va) | TC_map_or_big_map (ka, va)),
         ...
         |( (TC_big_map (ka, va) | TC_map_or_big_map (ka, va)),
            ...
            ok @@ ([ka; va], [kb; vb])
        | ( ( TC_option _ | TC_list _ | TC_contract _ | TC_set _ | TC_map _
            | TC_big_map _ | TC_map_or_big_map _ | TC_arrow _ ),
            ( TC_option _ | TC_list _ | TC_contract _ | TC_set _ | TC_map _
            | TC_big_map _ | TC_map_or_big_map _ | TC_arrow _ ) ) ->
      ...
      if List.length lsta <> List.length lstb then
        fail
        @@ different_operator_number_of_arguments opa opb (List.length lsta)
             (List.length lstb)
      else
...
let assert_literal_eq ((a, b) : literal * literal) : unit result =
  ...
  | Literal_bool _, Literal_bool _           -> fail
                                                @@ different_literals
                                                     "booleans" a b
  | Literal_bool _, _                          ->
...
let rec assert_value_eq ((a, b) : expression * expression) : unit result =
  ...
  | ( E_constant {cons_name= ca; arguments= lsta},
    ...
    when ca = cb ->
      let%bind lst =
        generic_try
          (different_size_values "constants with different number of elements"
             a b) (fun () -> List.combine lsta lstb)
      in
      let%bind _all = bind_list @@ List.map assert_value_eq lst in
      ok ()
  | E_constant _, E_constant _ -> fail @@ different_values "constants" a b
  | E_constant _, _ ->
      let error_content () =
  ...
  | E_constructor _, _ ->
      ...
      @@ different_values_because_different_types
           "constructor vs. non-constructor" a b
...
let merge_annotation
    (a : type_expression option)
    (b : type_expression option)

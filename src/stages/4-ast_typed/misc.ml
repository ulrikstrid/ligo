...
module Errors = struct
  ...
  let different_constants a b () =
    ...
    let message () =
      "Expected these two constant type constructors to be the same, but \
       they're different"
    in
    let data =
  ...
  let different_operators a b () =
    let title = thunk "different type constructors" in
    let message () =
      "Expected these two n-ary type constructors to be the same, but they're \
       different"
    in
    let data =
      [ ( "a",
          fun () ->
            Format.asprintf "%a" (PP.type_operator PP.type_expression) a );
        ( "b",
          fun () ->
            Format.asprintf "%a" (PP.type_operator PP.type_expression) b ) ]
    in
    error ~data title message ()
  ...
  let different_operator_number_of_arguments opa opb lena lenb () =
    ...
    let message () =
      Format.asprintf
        "Expected these two n-ary type constructors to be the same, but they \
         have different numbers of arguments (both use the %s type \
         constructor, but they have %d and %d arguments, respectively)"
        (type_operator_name opa)
        lena
        lenb
    in
    let data =
      [ ( "a",
          fun () ->
            Format.asprintf "%a" (PP.type_operator PP.type_expression) opa );
        ( "b",
          fun () ->
            Format.asprintf "%a" (PP.type_operator PP.type_expression) opb );
  ...
  let different_size_type names a b () =
    ...
    let message () =
      "Expected these two types to be the same, but they're different (both \
       are " ^ names ^ ", but with a different number of arguments)"
    in
    let data =
      [ ("a", fun () -> Format.asprintf "%a" PP.type_expression a);
        ("b", fun () -> Format.asprintf "%a" PP.type_expression b) ]
  ...
  let different_props_in_record a b ra rb ka kb () =
    let names () =
      if
        Stage_common.Helpers.is_tuple_lmap ra
        && Stage_common.Helpers.is_tuple_lmap rb
      then "tuples"
      else "records"
    in
  ...
  let different_kind_record_tuple a b ra rb () =
    let name_a () =
      if Stage_common.Helpers.is_tuple_lmap ra then "tuple" else "record"
    in
    let name_b () =
      if Stage_common.Helpers.is_tuple_lmap rb then "tuple" else "record"
    in
    let title () = "different keys in " ^ name_a () ^ " and " ^ name_b () in
    let message () =
      "Expected these two types to be the same, but they're different (one is \
       a " ^ name_a () ^ " and the other is a " ^ name_b () ^ ")"
    in
    let data =
      [ ("a", fun () -> Format.asprintf "%a" PP.type_expression a);
        ("b", fun () -> Format.asprintf "%a" PP.type_expression b) ]
  ...
  let different_types name a b () =
    ...
    let message () =
      "Expected these two types to be the same, but they're different"
    in
    let data =
      [ ("a", fun () -> Format.asprintf "%a" PP.type_expression a);
        ("b", fun () -> Format.asprintf "%a" PP.type_expression b) ]
  ...
  let missing_key_in_record_value k () =
    ...
    let message () = "" in
    let data = [("missing_key", fun () -> Format.asprintf "%s" k)] in
    error ~data title message ()
...
module Free_variables = struct
  ...
  let rec expression_content : bindings -> expression_content -> bindings =
   fun b ec ->
    ...
    | E_constant {arguments; _} ->
        unions @@ List.map self arguments
    | E_variable name -> (
      match mem name b with true -> empty | false -> singleton name )
    | E_application {lamb; args} ->
        unions @@ List.map self [lamb; args]
    | E_constructor {element; _} ->
        self element
    | E_record m ->
        unions @@ List.map self @@ LMap.to_list m
    | E_record_accessor {record; _} ->
        self record
    | E_record_update {record; update; _} ->
        union (self record) @@ self update
  ...
  and lambda : bindings -> lambda -> bindings =
   fun b l ->
  ...
  and matching_variant_case :
      ...
      (bindings -> a -> bindings) ->
      bindings ->
      (constructor' * expression_variable) * a ->
      bindings =
   fun f b ((_, n), c) -> f (union (singleton n) b) c

  and matching :
      type a.
      (bindings -> a -> bindings) ->
      bindings ->
      (a, 'var) matching_content ->
      bindings =
   fun f b m ->
    match m with
    ...
    | Match_list {match_nil = n; match_cons = (hd, tl, c, _)} ->
        union (f b n) (f (union (of_list [hd; tl]) b) c)
    | Match_option {match_none = n; match_some = (opt, s, _)} ->
        union (f b n) (f (union (singleton opt) b) s)
    | Match_tuple ((lst, a), _) ->
        f (union (of_list lst) b) a
    | Match_variant (lst, _) ->
        unions @@ List.map (matching_variant_case f b) lst
...
let rec assert_type_expression_eq ((a, b) : type_expression * type_expression)
  ...
  | (T_constant ca, T_constant cb) ->
      trace_strong (different_constants ca cb) @@ Assert.assert_true (ca = cb)
  | (T_constant _, _) ->
      fail @@ different_kinds a b
  | (T_operator opa, T_operator opb) ->
      let%bind (lsta, lstb) =
        ...
        | (TC_set la, TC_set lb) ->
            ok @@ ([la], [lb])
        | ( (TC_map (ka, va) | TC_map_or_big_map (ka, va)),
            (TC_map (kb, vb) | TC_map_or_big_map (kb, vb)) )
        | ( (TC_big_map (ka, va) | TC_map_or_big_map (ka, va)),
            (TC_big_map (kb, vb) | TC_map_or_big_map (kb, vb)) ) ->
            ok @@ ([ka; va], [kb; vb])
        | ( ( TC_option _
            | TC_list _
            | TC_contract _
      ...
      if List.length lsta <> List.length lstb then
        fail
        @@ different_operator_number_of_arguments
             opa
             opb
             (List.length lsta)
             (List.length lstb)
        ...
        @@ bind_list_iter
             (fun (a, b) -> assert_type_expression_eq (a, b))
  ...
  | (T_sum sa, T_sum sb) ->
      ...
      let aux ((ka, va), (kb, vb)) =
        let%bind _ =
          Assert.assert_true ~msg:"different keys in sum types" @@ (ka = kb)
        in
        assert_type_expression_eq (va, vb)
      in
      let%bind _ =
        trace_strong (different_size_sums a b)
        @@ Assert.assert_list_same_size sa' sb'
      in
  ...
  | (T_record ra, T_record rb)
    when Stage_common.Helpers.is_tuple_lmap ra
         <> Stage_common.Helpers.is_tuple_lmap rb ->
      fail @@ different_kind_record_tuple a b ra rb
  | (T_record ra, T_record rb) ->
      let sort_lmap r' =
        List.sort (fun (Label a, _) (Label b, _) -> String.compare a b) r'
      in
      ...
      let aux ((ka, va), (kb, vb)) =
        let%bind _ =
          ...
          @@ Assert.assert_true (ka = kb)
        in
        assert_type_expression_eq (va, vb)
      in
      let%bind _ =
        trace_strong (different_size_records_tuples a b ra rb)
        @@ Assert.assert_list_same_size ra' rb'
      in
  ...
  | (T_variable x, T_variable y) ->
      let _ = x = y in
      failwith
        "TODO : we must check that the two types were bound at the same \
         location (even if they have the same name), i.e. use something like \
         De Bruijn indices or a propper graph encoding"
  | (T_variable _, _) ->
      fail @@ different_kinds a b
...
let assert_literal_eq ((a, b) : literal * literal) : unit result =
  ...
  | (Literal_bool _, Literal_bool _) ->
      fail @@ different_literals "booleans" a b
  | (Literal_bool _, _) ->
  ...
  | (Literal_bytes _, Literal_bytes _) ->
      fail @@ different_literals "different bytes" a b
  | (Literal_bytes _, _) ->
      fail
...
let rec assert_value_eq ((a, b) : expression * expression) : unit result =
  ...
  | ( E_constant {cons_name = ca; arguments = lsta},
      E_constant {cons_name = cb; arguments = lstb} )
    when ca = cb ->
      let%bind lst =
        generic_try
          (different_size_values
             "constants with different number of elements"
             a
             b)
  ...
  | (E_constant _, _) ->
      ...
      fail
      @@ fun () ->
      error (thunk "comparing constant with other stuff") error_content ()
  | ( E_constructor {constructor = ca; element = a},
      E_constructor {constructor = cb; element = b} )
    when ca = cb ->
  ...
  | (E_constructor _, _) ->
      fail
      @@ different_values_because_different_types
  ...
  | (E_record sma, E_record smb) ->
      let aux (Label k) a b =
        ...
        | _ ->
            Some (fail @@ missing_key_in_record_value k)
      in
      let%bind _all =
        Stage_common.Helpers.bind_lmap @@ LMap.merge aux sma smb
      in
      ok ()
  | (E_record _, _) ->
      fail
      @@ different_values_because_different_types "record vs. non-record" a b
  | (E_literal _, _)
  | (E_variable _, _)
  | (E_application _, _)
  | (E_lambda _, _)
...
let merge_annotation (a : type_expression option) (b : type_expression option)
    err : type_expression result =
  match (a, b) with
  | (None, None) ->
      fail @@ err
  | (Some a, None) ->
...
let get_entry (lst : program) (name : string) : expression result =
  ...
  let aux x =
    ...
    if an = Var.of_name name then Some expr else None
  in
...
let program_environment (program : program) : full_environment =
  ...
  | Declaration_constant (_, _, _, post_env) ->
      post_env

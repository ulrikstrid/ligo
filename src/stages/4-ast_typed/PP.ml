...
and expression_content ppf (ec : expression_content) =
  ...
  | E_record_update {record; path; update} ->
      fprintf ppf "{ %a with { %a = %a } }" expression record label path
        expression update
  | E_lambda {binder; result} ->
      fprintf ppf "lambda (%a) return %a" expression_variable binder expression
        result
  | E_matching {matchee; cases} ->
      fprintf ppf "match %a with %a" expression matchee (matching expression)
        cases
  | E_let_in {let_binder; rhs; let_result; inline} ->
      fprintf ppf "let %a = %a%a in %a" expression_variable let_binder
...
and matching :
    type a. (formatter -> a -> unit) -> _ -> (a, 'var) matching_content -> unit
    =
 fun f ppf m ->
   match m with
   | Match_tuple ((lst, b), _) ->
   ...
   | Match_list {match_nil; match_cons= hd_name, tl_name, match_cons, _} ->
       fprintf ppf "| Nil -> %a @.| %a :: %a -> %a" f match_nil
         expression_variable hd_name expression_variable tl_name f match_cons
...
let declaration ppf (d : declaration) =
  ...
  | Declaration_constant (name, expr, inline, _) ->
      fprintf ppf "const %a = %a%a" expression_variable name expression expr

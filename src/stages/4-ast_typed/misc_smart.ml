...
let program_to_main : program -> string -> lambda result =
 fun p s ->
  let%bind (main, input_type, _) =
    let pred d =
      match d with
      | Declaration_constant (d, expr, _, _) when d = Var.of_name s ->
          Some expr
      | Declaration_constant _ ->
          None
    in
    let%bind main =
      trace_option (simple_error "no main with given name")
      @@ List.find_map (Function.compose pred Location.unwrap) p
    in
    let%bind (input_ty, output_ty) =
      ...
      | T_arrow {type1; type2} ->
          ok (type1, type2)
      | _ ->
          simple_fail "program main isn't a function"
    in
    ok (main, input_ty, output_ty)
  in
  let env =
    let aux _ d =
      match d with Declaration_constant (_, _, _, post_env) -> post_env
    in
    List.fold_left aux Environment.full_empty (List.map Location.unwrap p)
  in
  let binder = Var.of_name "@contract_input" in
  let result =
    let input_expr = e_a_variable binder input_type env in
    let main_expr =
      e_a_variable (Var.of_name s) (get_type_expression main) env
    in
    e_a_application main_expr input_expr env
  in
  ok {binder; result}
...
module Captured_variables = struct
  ...
  let rec expression : bindings -> expression -> bindings result =
   fun b e -> expression_content b e.environment e.expression_content

  and expression_content :
      bindings -> full_environment -> expression_content -> bindings result =
   fun b env ec ->
    let self = expression b in
    match ec with
    | E_lambda l ->
        ok @@ Free_variables.lambda empty l
    | E_literal _ ->
        ok empty
    ...
    | E_variable name -> (
        let%bind env_element =
          ...
          @@ Environment.get_opt name env
        in
        match env_element.definition with
        | ED_binder ->
            ok empty
        | ED_declaration {expr = _; free_variables = _} ->
            simple_fail "todo" )

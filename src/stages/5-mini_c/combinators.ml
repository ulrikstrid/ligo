...
let get_bool (v : value) =
  match v with D_bool b -> ok b | _ -> simple_fail "not a bool"

let get_int (v : value) =
  match v with D_int n -> ok n | _ -> simple_fail "not an int"

let get_nat (v : value) =
  match v with D_nat n -> ok n | _ -> simple_fail "not a nat"

let get_mutez (v : value) =
  match v with D_mutez n -> ok n | _ -> simple_fail "not a mutez"

let get_timestamp (v : value) =
  match v with D_timestamp n -> ok n | _ -> simple_fail "not a timestamp"

let get_string (v : value) =
  match v with D_string s -> ok s | _ -> simple_fail "not a string"

let get_bytes (v : value) =
  match v with D_bytes b -> ok b | _ -> simple_fail "not a bytes"

let get_unit (v : value) =
  match v with D_unit -> ok () | _ -> simple_fail "not a unit"
...
let get_big_map (v : value) =
  match v with D_big_map lst -> ok lst | _ -> simple_fail "not a big_map"

let get_list (v : value) =
  match v with D_list lst -> ok lst | _ -> simple_fail "not a list"

let get_set (v : value) =
  match v with D_set lst -> ok lst | _ -> simple_fail "not a set"
...
let get_t_option (v : type_value) =
  match v with T_option t -> ok t | _ -> simple_fail "not an option"

let get_pair (v : value) =
  match v with D_pair (a, b) -> ok (a, b) | _ -> simple_fail "not a pair"
...
let get_t_big_map (t : type_value) =
  match t with T_big_map kv -> ok kv | _ -> simple_fail "not a type big_map"

let get_t_list (t : type_value) =
  match t with T_list t -> ok t | _ -> simple_fail "not a type list"

let get_t_set (t : type_value) =
  match t with T_set t -> ok t | _ -> simple_fail "not a type set"

let get_left (v : value) =
  match v with D_left b -> ok b | _ -> simple_fail "not a left"

let get_right (v : value) =
  match v with D_right b -> ok b | _ -> simple_fail "not a right"
...
let get_t_left t =
  match t with T_or ((_, a), _) -> ok a | _ -> fail @@ wrong_type "union" t

let get_t_right t =
  match t with T_or (_, (_, b)) -> ok b | _ -> fail @@ wrong_type "union" t

let get_t_contract t =
  match t with T_contract x -> ok x | _ -> fail @@ wrong_type "contract" t
...
let e_let_in v tv inline expr body : expression =
  Expression.(make_tpl (E_let_in ((v, tv), inline, expr, body), get_type body))

let ez_e_sequence a b : expression =
  Expression.(make_tpl (E_sequence (make_tpl (a, t_unit), b), get_type b))

let d_unit : value = D_unit

let environment_wrap pre_environment post_environment =

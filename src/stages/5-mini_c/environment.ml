...
module Environment (* : ENVIRONMENT *) = struct
  ...
  let get_opt : expression_variable -> t -> type_value option =
    List.assoc_opt ~compare:Var.compare

  let has : expression_variable -> t -> bool =
   fun s t -> match get_opt s t with None -> false | Some _ -> true

  let get_i : expression_variable -> t -> type_value * int =
    List.assoc_i ~compare:Var.compare

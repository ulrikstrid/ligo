...
module Free_variables = struct
  ...
  let rec expression : bindings -> expression -> bindings =
   fun b e ->
    ...
    | E_skip ->
        empty
    | E_constant c ->
        unions @@ List.map self c.arguments
    | E_application (f, x) ->
        unions @@ [self f; self x]
    | E_variable n ->
        var_name b n
    | E_make_none _ ->
        empty
  ...
  and var_name : bindings -> var_name -> bindings =
   fun b n -> if mem n b then empty else singleton n
  ...
  and value : bindings -> value -> bindings =
   fun b v ->
    ...
    | D_operation _ ->
        empty
    | D_pair (x, y) ->
        unions [self x; self y]
    | D_left x | D_right x | D_some x ->
        self x
...
let get_entry (lst : program) (name : string) : (expression * int) result =
  let%bind entry_expression =
    ...
    let aux x =
      ...
      if Var.equal decl_name (Var.of_name name) then Some decl_expr else None
    in
...
let aggregate_entry (lst : program) (form : form_t) : expression result =
  ...
  | ContractForm entry_expression -> (
    ...
    | E_closure l ->
        ...
        ok e'
    | _ ->
        simple_fail "a contract must be a closure" )

...
module Errors = struct
  ...
  let not_functional_main name =
    ...
    let data = [("name", fun () -> Format.asprintf "%s" name)] in
    error ~data title content
end
...
module Free_variables = struct
  ...
  let mem_count : expression_variable -> bindings -> int =
   fun x fvs -> List.length (List.filter (Var.equal x) fvs)
  ...
  let rec expression : bindings -> expression -> bindings =
   fun b e ->
     ...
     | E_while (cond, body)                           -> union (self cond)
                                                           (self body)
  ...
  and value : bindings -> value -> bindings =
   fun b v ->
     let self = value b in
     match v with
     | D_unit | D_bool _ | D_nat _ | D_timestamp _ | D_mutez _ | D_int _
      |D_string _ | D_bytes _ | D_none | D_operation _ ->
         empty
     | D_pair (x, y) -> unions [self x; self y]
     | D_left x | D_right x | D_some x -> self x
     | D_map kvs | D_big_map kvs ->
         unions @@ List.map (fun (k, v) -> unions [self k; self v]) kvs
     | D_list xs | D_set xs -> unions @@ List.map self xs
...
let get_entry (lst : program) (name : string) : (expression * int) result =
  let%bind entry_expression =
    ...
    let aux x =
      ...
      if Var.equal decl_name (Var.of_name name) then
        Some decl_expr
      else
        None
    in
...
type form_t = ContractForm of expression | ExpressionForm of expression
...
let aggregate_entry (lst : program) (form : form_t) : expression result =
  ...
  | ContractForm entry_expression   -> (
    ...
    | E_closure l ->
        ...
        let e' =
          {content= E_closure l'; type_value= entry_expression.type_value}
        in
        ok e'
    | _           -> simple_fail "a contract must be a closure" )
  | ExpressionForm entry_expression -> ok @@ wrapper entry_expression

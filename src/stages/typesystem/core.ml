...
let fresh_type_variable : ?name:string -> unit -> type_variable = Var.fresh
...
and simple_c_constructor = constant_tag * type_variable list

(* non-empty list *)
and simple_c_constant = constant_tag

(* for type constructors that do not take arguments *)
...
(* poor man's type-level computation to ensure that TV.label is type_variable *)

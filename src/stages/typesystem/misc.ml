...
module Substitution = struct
  module Pattern = struct
    ...
    type substs = variable:string -> T.type_value' option (* this string is a type_name or type_variable I think *)
    ...
    and s_environment_element_definition ~substs = function
      ...
      | T.ED_declaration (val_, free_variables) ->
        ...
        let%bind free_variables = bind_map_list (s_type_variable ~substs) free_variables in
        ok @@ T.ED_declaration (val_, free_variables)
    and s_environment ~substs = fun lst ->
    ...
    and s_variable : T.name w = fun ~substs var ->
      let () = ignore @@ substs in
      ok var

    and s_type_variable : T.name w = fun ~substs tvar ->
    ...
    and s_type_name_constant : T.type_name w = fun ~substs type_name ->
    ...
    and s_type_value' : T.type_value' w = fun ~substs -> function
        ...
        | T.T_variable (Type_name variable) ->
           begin
             ...
             | None -> ok @@ T.T_variable (Type_name variable)
           end
        | T.T_function _ ->
          let _TODO = substs in
          failwith "TODO: T_function"

    and s_type_expression : Ast_simplified.type_expression w = fun ~substs -> function
      ...
      | Ast_simplified.T_constant (variable, te_list) ->
         let () =
           begin
             match substs ~variable with
             | Some _ ->
                Printf.printf "TODO: s_type_expression variable : not substituing, is this the right thing to do?"
             | None -> ()
           end
         in
         let variable : string = variable in
         let%bind te_list = bind_map_list (s_type_expression ~substs) te_list in
    ...
    and s_expression : T.expression w = fun ~(substs : substs) -> function
      ...
      | T.E_constructor     (tvar, val_) ->
        let%bind tvar = s_type_variable ~substs tvar in
        let%bind val_ = s_annotated_expression ~substs val_ in
    ...
    and s_named_expression : T.named_expression w = fun ~substs { name; annotated_expression } ->
      let%bind name = s_type_variable ~substs name in

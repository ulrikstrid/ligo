...
module Substitution = struct
  module Pattern = struct
    ...
    and s_environment : (T.environment,_) w = fun ~substs env ->
    ...
    and s_small_environment : (T.small_environment,_) w = fun ~substs T.{expression_environment ; type_environment} ->
    ...
    and s_full_environment : (T.full_environment,_) w = fun ~substs (a , b) ->
    ...
    and s_type_variable : (T.type_variable,_) w = fun ~substs tvar ->
    ...
    and s_abstr_type_expression : (Ast_core.type_expression,_) w = fun ~substs {type_content;type_meta} ->
      let%bind type_content = s_abstr_type_content ~substs type_content in
      ok @@ Ast_core.{type_content;type_meta}

    and s_type_expression : (T.type_expression,_) w = fun ~substs { type_content; type_meta } ->
    ...
    and s_accessor  : (T.accessor,_) w = fun ~substs _ ->
      let _TODO = substs in
    ...
    and s_expression : (T.expression,_) w = fun ~(substs:substs) { expression_content; type_expression; environment; location } ->
